...
`deepagents` 库还暴露了 [create_summarization_tool_middleware](https://reference.langchain.com/python/deepagents/middleware/summarization/create_summarization_tool_middleware)，使代理能够在适当的时候触发摘要——例如在任务之间——而不是在固定的令牌间隔。有关更多详细信息，请参阅 [Harness 中的摘要](/oss/python/deepagents/harness#summarization)。
