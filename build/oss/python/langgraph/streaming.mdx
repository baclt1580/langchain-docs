...
LangGraph 图暴露了 [`stream`](https://reference.langchain.com/python/langgraph/pregel/#langgraph.pregel.Pregel.stream) (同步) 和 [`astream`](https://reference.langchain.com/python/langgraph/pregel/#langgraph.pregel.Pregel.astream) (异步) 方法来生成流式输出作为迭代器。传递一个或多个 [流模式](#stream-modes) 来控制您接收的数据。
...
| 模式 | 类型 | 描述 |
| :--- | :--- | :---------- |
| [values](#graph-state) | [`ValuesStreamPart`](https://reference.langchain.com/python/langgraph/types/ValuesStreamPart) | 每一步后的完整状态。 |
| [updates](#graph-state) | [`UpdatesStreamPart`](https://reference.langchain.com/python/langgraph/types/UpdatesStreamPart) | 每一步后的状态更新。同一步骤中的多个更新将单独流式传输。 |
| [messages](#llm-tokens) | [`MessagesStreamPart`](https://reference.langchain.com/python/langgraph/types/MessagesStreamPart) | LLM 调用的 (LLM 令牌，元数据) 2-元组。 |
...
| [checkpoints](#checkpoints) | [`CheckpointStreamPart`](https://reference.langchain.com/python/langgraph/types/CheckpointStreamPart) | 检查点事件（与 `get_state()` 格式相同）。需要检查点器。 |
| [tasks](#tasks) | [`TasksStreamPart`](https://reference.langchain.com/python/langgraph/types/TasksStreamPart) | 带有结果和错误的任务开始/结束事件。需要检查点器。 |
...
当在 Python < 3.11 中使用异步代码时，您必须显式传递 [`RunnableConfig`](https://reference.langchain.com/python/langchain-core/runnables/config/RunnableConfig) 给 `ainvoke()` 以启用正确的流式传输。详见 [Python < 3.11 的异步](#async) 或升级到 Python 3.11+。
</Warning>
...
<Warning>
**Python < 3.11 的异步中没有 [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/get_stream_writer)**
...
要将 [子图](/oss/python/langgraph/use-subgraphs) 的输出包含在流式输出中，您可以在父图的 `.stream()` 方法中设置 `subgraphs=True`。这将流式传输来自父图和任何子图的输出。
...
使用 `checkpoints` 流模式在图执行期间接收检查点事件。每个检查点事件的格式与 `get_state()` 的输出相同。需要 [检查点器](/oss/python/langgraph/persistence)。
...
使用 `tasks` 流模式在图执行期间接收任务开始和结束事件。任务事件包含有关正在运行的节点、其结果和任何错误的信息。需要 [检查点器](/oss/python/langgraph/persistence)。
...
当向 `invoke()` 或 `ainvoke()` 传递 `version="v2"` 时，它返回一个 [`GraphOutput`](https://reference.langchain.com/python/langgraph/types/GraphOutput) 对象，包含 `.value` 和 `.interrupts` 属性：
...
2. 您**无法**在异步节点或工具中使用 [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/get_stream_writer) — 您必须直接传递 `writer` 参数。
