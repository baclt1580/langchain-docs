...
description: "使用 LangChain Python 与 BigTableByteStore 存储进行集成。"
---
...
`BigtableByteStore` 使用 Google Cloud Bigtable 作为键值存储的后端。它支持同步和异步操作来设置、获取和删除键值对。
...
开始之前，您需要一个拥有活跃 Bigtable 实例和表的 Google Cloud 项目。
...
集成位于 `langchain-google-bigtable` 包中。下面的命令也会为嵌入缓存示例安装 `langchain-google-vertexai`。
...
设置您的 Google Cloud 项目以在笔记本中使用其资源。
...
### BigtableEngine
`BigtableEngine` 对象处理存储的执行上下文，特别是用于异步操作。建议初始化单个引擎并在多个存储之间重用它以获得更好的性能。
...
### 获取
使用 `amget` 检索值。如果找不到键，则返回该键的 `None`。
...
本节展示如何使用 Bigtable 存储创建简单的检索器。它充当文档持久层，获取匹配查询前缀的文档。
...
有关 `BigtableByteStore` 类的完整详细信息，请查看 [GitHub](https://github.com/googleapis/langchain-google-bigtable-python/blob/main/src/langchain_google_bigtable/key_value_store.py) 上的源代码。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\stores\bigtable.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
