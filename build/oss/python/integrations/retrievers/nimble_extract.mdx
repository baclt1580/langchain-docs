...
>[Nimble 的 Extract API](https://docs.nimbleway.com/nimble-sdk/extract-api) 通过无头浏览器浏览特定 URL 来提取渲染内容，而不是依赖缓存或受 API 限制的数据。此检索器处理 JavaScript 渲染、动态内容和复杂的导航流程——使其适用于需要访问特定网页的 RAG 应用程序，包括分页、过滤器和客户端渲染背后的内容。

我们可以将其用作 [检索器](/oss/python/langchain/retrieval)。它将展示此集成的特定功能。浏览完毕后，探索 [相关用例页面](/oss/python/langchain/rag) 以了解如何将此检索器作为更大链的一部分使用可能很有用。
...
我们可以轻松地将此检索器组合到 RAG 链中，用于提取和分析特定的 Web 内容：
...
- **无需等待** (`wait=None`)：大多数现代网站的默认设置
- **短等待** (`wait=1000-2000`)：用于具有懒加载或延迟内容的页面
- **更长等待** (`wait=5000+`)：用于加载缓慢的 SPA 或需要时间完全渲染的重型 JavaScript
...
2. **批量相关 URL**：并行提取同一域中的多个页面
3. **选择合适的格式**：RAG 使用 Markdown，简单处理使用 plain_text
