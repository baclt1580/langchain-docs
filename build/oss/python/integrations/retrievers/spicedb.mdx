...
`SpiceDBRetriever` 是 LangChain 的 `BaseRetriever`，它用 SpiceDB 授权包装了任何现有的检索器。它遵循后过滤授权模式：首先基于语义搜索检索文档，然后根据用户权限进行过滤。
...
<Tip>
    此检索器需要一个正在运行的 SpiceDB 实例。有关设置说明，请参阅 [SpiceDB 提供者页面](/oss/python/integrations/providers/spicedb)。
</Tip>
...
- **base_retriever** (BaseRetriever): 要包装授权的底层检索器（必需）
- **subject_id** (str): 检查权限的用户 ID（必需）
...
- **resource_id_key** (str): 包含资源 ID 的文档元数据键（默认："resource_id"）
- **fail_open** (bool): 如果为 True，允许在出错时访问；如果为 False，在出错时拒绝访问（默认：False）
- **use_tls** (bool): 是否对 SpiceDB 连接使用 TLS（默认：False）

<Warning>
  **所有参数对于 SpiceDB 做出访问决策都是必需的。** 虽然有些有默认值，但您应该显式设置它们以匹配您的 SpiceDB 架构。
</Warning>
...
该检索器使用 SpiceDB 原生的 `CheckBulkPermissionsRequest` API 以获得最佳性能：

- **单次 API 调用**：所有权限在一个请求中检查，而不是 N 个单独调用
- **高效**：比单独的权限检查快得多
- **可扩展**：能高效处理数百个文档
...
默认情况下，检索器会失败关闭——如果在检查权限时出错，文档将被过滤掉：
...
**方法**：
- `invoke(query: str) -> List[Document]`: 同步检索授权文档
- `ainvoke(query: str) -> List[Document]`: 异步检索授权文档
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\spicedb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
