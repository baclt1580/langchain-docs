...
>[DocArray](https://github.com/docarray/docarray) 是一个多功能、开源的工具，用于管理您的多模态数据。它允许您按需塑造数据，并提供灵活性以使用各种文档索引后端存储和搜索它。此外，它变得更棒——您可以利用您的 `DocArray` 文档索引创建 `DocArrayRetriever`，并构建出色的 LangChain 应用！

本笔记本分为两个部分。[第一部分](#document-index-backends) 介绍了所有五种支持的文档索引后端。它提供了设置和索引每个后端的指导，并指示您如何构建 `DocArrayRetriever` 以查找相关文档。
...
在开始构建索引之前，定义您的文档模式非常重要。这决定了您的文档将有哪些字段以及每个字段将保存什么类型的数据。

对于这个演示，我们将创建一个包含 'title' (str), 'title_embedding' (numpy array), 'year' (int) 和 'color' (str) 的随机模式
...
`InMemoryExactNNIndex` 将所有文档存储在内存中。它是小型数据集的良好起点，在那里您可能不想启动数据库服务器。
...
`HnswDocumentIndex` 是一个轻量级的文档索引实现，完全在本地运行，最适合小到中等规模的数据集。它将向量存储在 [hnswlib](https://github.com/nmslib/hnswlib) 中，并将所有其他数据存储在 [SQLite](https://www.sqlite.org/index.html) 中。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\docarray_retriever.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
