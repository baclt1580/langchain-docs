...
description: "使用 LangChain Python 与 Google Drive 检索器进行集成。"
---
...
`token.json` 的位置使用相同的目录（或使用参数 `token_path`）。请注意，首次使用检索器时会自动创建 `token.json`。

`GoogleDriveRetriever` 可以通过一些请求检索选定的文件。

默认情况下，如果您使用 `folder_id`，此文件夹内的所有文件都可以被检索为 `Document`。
...
默认情况下，具有这些 MIME 类型的所有文件都可以转换为 `Document`。
...
可以更新或自定义此内容。请参见 `GoogleDriveRetriever` 的文档。
...
| `gdrive-query-in-folder`                 | 在 `folder_id` 中搜索 `query`（并在 `_recursive=true` 中包含子文件夹）  |
...
否则，您可以使用专用的 `PromptTemplate` 自定义提示词
...
每个 Google Drive 在元数据中都有一个 `description` 字段（请参阅*文件详情*）。
使用 `snippets` 模式返回选定文件的描述。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\google_drive.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
