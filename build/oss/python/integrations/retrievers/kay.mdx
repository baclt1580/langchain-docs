...
>[Kai Data API](https://www.kay.ai/) 专为 RAG 构建 🕵️ 我们正在整理世界上最大的数据集作为高质量嵌入，以便您的 AI 代理可以即时检索上下文。最新模型、快速检索和零基础设施。

本笔记本向您展示如何检索 [Kay](https://kay.ai/) 支持的数据集。您目前可以搜索 `SEC Filings` 和 `Press Releases of US companies`。访问 [kay.ai](https://kay.ai) 获取最新数据发布。如有任何问题，加入我们的 [discord](https://discord.gg/hAnE4e5T6M) 或 [向我们发推](https://twitter.com/vishalrohra_)
...
您还需要一个 API 密钥：您可以在 [https://kay.ai](https://kay.ai/) 免费获取一个。一旦获得 API 密钥，必须将其设置为环境变量 `KAY_API_KEY`。

`KayAiRetriever` 有一个静态 `.create()` 工厂方法，接受以下参数：

* `dataset_id: string` 必需 -- Kay 数据集 ID。这是关于特定实体（如公司、个人或地点）的数据集合。例如，尝试 `"company"`
* `data_type: List[string]` 可选 -- 这是基于来源或格式的数据集内的类别，例如“公司”数据集内的‘SEC Filings'、‘Press Releases'或‘Reports'。例如，在“公司”数据集下尝试 ["10-K", "10-Q", "PressRelease"]。如果留空，Kay 将检索所有类型中最相关的上下文。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\kay.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
