---
title: "SEC 备案集成"
description: "使用 LangChain Python 与 SEC 备案检索器进行集成。"
---

>[SEC filing](https://www.sec.gov/edgar) 是向美国证券交易委员会（SEC）提交的财务报表或其他正式文件。上市公司、某些内部人士和经纪交易商需要定期提交 `SEC filings`。投资者和金融专业人士依赖这些备案来获取他们正在评估投资目的的公司信息。
>
>`SEC filings` 数据由 [Kay.ai](https://kay.ai) 和 [Cybersyn](https://www.cybersyn.com/) 通过 [Snowflake Marketplace](https://app.snowflake.com/marketplace/providers/GZTSZAS2KCS/Cybersyn%2C%20Inc) 提供。

## 设置

首先，您需要安装 `kay` 包。您还需要一个 API 密钥：您可以在 [https://kay.ai](https://kay.ai/) 免费获取一个。一旦获得 API 密钥，您必须将其设置为环境变量 `KAY_API_KEY`。

在本示例中，我们将使用 `KayAiRetriever`。查看 [kay notebook](/oss/python/integrations/retrievers/kay) 以获取更多关于其接受参数的详细信息。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\sec_filings.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
