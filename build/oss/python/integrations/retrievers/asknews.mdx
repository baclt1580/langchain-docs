...
> [AskNews](https://asknews.app) 为任何 LLM 注入最新的全球新闻（或历史新闻），只需一条自然语言查询。具体来说，AskNews 每天通过翻译、摘要、提取实体并将它们索引到热和冷向量数据库中，来丰富超过 30 万篇文章。AskNews 将这些向量数据库置于低延迟端点上供您使用。当您查询 AskNews 时，您会得到一个经过提示优化的字符串，其中包含所有最相关的增强信息（例如实体、分类、翻译、摘要）。这意味着您不需要管理自己的新闻 RAG，也不需要担心如何以浓缩的方式向您的 LLM 传达新闻信息。
> AskNews 也致力于透明度，这就是为什么我们的覆盖范围在数百个国家、13 种语言和 5 万个来源上进行监控和多样化。如果您想跟踪我们的来源覆盖范围，可以访问我们的 [透明度仪表板](https://asknews.app/en/transparency)。
...
我们还需要设置我们的 AskNews API 凭据，这可以在 [AskNews 控制台](https://my.asknews.app) 生成。
...
设置 [LangSmith](https://smith.langchain.com/) 也有帮助（但不是必需的），以获得一流的观测能力。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\asknews.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
