...
description: "使用 LangChain Python 集成 LinkupSearchRetriever 检索器。"
---

> [Linkup](https://www.linkup.so/) 提供 API，将 LLM 连接到网络以及 Linkup Premium Partner 来源。
...
| 检索器 | 来源 | 包 |
| :--- | :--- | :---: |
`LinkupSearchRetriever` | 网络和伙伴来源 | langchain-linkup |

## 设置

要使用 Linkup 提供商，您需要一个有效的 API 密钥，您可以通过 [注册 Linkup](https://app.linkup.so/sign-up) 找到它。然后您可以将其设置为 `LINKUP_API_KEY` 环境变量。对于下面的链示例，您还需要将 OpenAI API 密钥设置为 `OPENAI_API_KEY` 环境变量，您也可以在此处完成：
...
如果您想要从单个查询中获得自动跟踪，您还可以通过取消注释以下内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\linkup_search.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
