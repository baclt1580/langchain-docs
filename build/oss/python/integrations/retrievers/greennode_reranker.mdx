...
>[GreenNode](https://greennode.ai/) 是一家全球 AI 解决方案提供商，也是 **NVIDIA 首选合作伙伴**，为美国、中东及北非（MENA）和亚太地区的企业提供从基础设施到应用的全栈 AI 能力。依托 **世界级的基础设施**（LEED Gold, TIA‑942, Uptime Tier III），GreenNode 为企业、初创公司和研究人员赋能，提供全面的 AI 服务套件

本指南提供了关于如何使用 `GreenNodeRerank` 检索器的入门教程。它使您能够使用内置连接器或通过集成自己的数据源执行文档搜索，利用 GreenNode 的重排序功能以提高相关性。
...
- **评分**: 返回相关性分数，用于根据查询对齐情况重新排列文档候选项
...
前往 [此页面](https://aiplatform.console.greennode.ai/api-keys) 注册 GreenNode AI Platform 并生成 API 密钥。完成后，设置 GREENNODE_API_KEY 环境变量：
...
如果您希望从单个查询中获得自动追踪，您也可以取消注释以下内容以设置您的 [LangSmith](/langsmith/home) API 密钥：
...
`GreenNodeRerank` 类可以使用可选参数进行实例化，这些参数包括 API 密钥和模型名称：
...
重排序模型通过根据语义相关性细化和重新排列初始搜索结果来增强检索增强生成 (RAG) 工作流。下面的示例展示了如何将 GreenNodeRerank 与基础检索器集成，以提高检索文档的质量。
...
`GreenNodeRerank` 类可以独立使用，根据相关性分数对检索到的文档进行重排序。此功能在以下场景中特别有用：当主要检索步骤（例如关键字或向量搜索）返回一组广泛的候选项时，需要一个次要模型使用更复杂的语义理解来细化结果。该类接受一个查询和一个候选文档列表，并返回基于预测相关性的重排序列表。
...
GreenNodeRerank 可以在 LangChain RAG 管道中无缝工作。以下是使用 GreenNodeRerank 创建简单 RAG 链的示例：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\retrievers\greennode_reranker.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
