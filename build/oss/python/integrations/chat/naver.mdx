...
本指南提供了通过 CLOVA Studio 开始使用 Naver HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models)的快速概述。有关 `ChatClovaX` 所有功能和配置的详细文档，请参阅 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。

[CLOVA Studio](http://clovastudio.ncloud.com/) 提供多种聊天模型。您可以在 CLOVA Studio 指南 [文档](https://guide.ncloud-docs.com/docs/clovastudio-model) 中找到最新模型的信息，包括其成本、上下文窗口和支持的输入类型。
...
3. 创建要使用模型的 CLOVA Studio 测试应用或服务应用（参见 [CLOVA Studio 测试应用设置](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp)。）
4. 签发测试或服务 API 密钥（参见 [Naver API 密钥文档](https://api.ncloud-docs.com/docs/ai-naver-clovastudio-summary#API%ED%82%A4)。）
...
class Weather(BaseModel):
    ...
    precipitation_percent: int | None = Field(
        default=None,
        description="降水概率百分比（可选，可以为 None）",
    )
```

**注意**：CLOVA Studio 支持使用 json 模式方法的结构化输出。将 `method` 设置为 `json_schema`。
