...
2.  **部署聊天模型端点：** 按照 [RunPod 提供商指南](/oss/python/integrations/providers/runpod#setup) 中的设置步骤，在 RunPod Serverless 上部署一个兼容的聊天模型端点，并获取其端点 ID。
...
if "RUNPOD_ENDPOINT_ID" not in os.environ:
    os.environ["RUNPOD_ENDPOINT_ID"] = input(
        "输入你的 RunPod 端点 ID（当 RUNPOD_CHAT_ENDPOINT_ID 未设置时使用）："
    )

# 可选：为聊天模型使用不同的端点 ID
# if "RUNPOD_CHAT_ENDPOINT_ID" not in os.environ:
#     os.environ["RUNPOD_CHAT_ENDPOINT_ID"] = input("输入你的 RunPod 聊天端点 ID（可选）：")
...
初始化 `ChatRunPod` 类。你可以通过 `model_kwargs` 传递模型特定参数，并配置轮询行为。
...
messages = [
    ...
    HumanMessage(content="RunPod Serverless API 的流程是什么？"),
]
...
高级功能的可用性**高度依赖**于你的 RunPod 端点处理器的具体实现。`ChatRunPod` 集成提供了基本框架，但处理器必须支持底层功能。
...
| 异步调用                                                     | ✅       | ✅           | 核心 `ainvoke`/`astream` 已实现。依赖于端点处理器的性能。                                                                                                                 |
| 令牌计数                                                     | ❌       | ✅           | 需要处理器在最终响应中返回 `prompt_tokens`、`completion_tokens`。集成当前未解析此信息。                                                                                   |
| 对数概率                                                     | ❌       | ✅           | 需要处理器返回对数概率。集成当前未解析此信息。                                                                                                                            |

**关键要点：** 如果端点遵循基本的 RunPod API 约定，标准的聊天调用和模拟流式传输可以工作。高级功能需要特定的处理器实现，并可能需要扩展或自定义此集成包。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\chat\runpod.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
