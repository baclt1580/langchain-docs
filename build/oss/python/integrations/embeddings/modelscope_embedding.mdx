...
ModelScope ([首页](https://www.modelscope.cn/) | [GitHub](https://github.com/modelscope/modelscope)) 基于“模型即服务”（MaaS）的理念构建。它致力于汇聚人工智能社区中最先进的机器学习模型，并简化在现实应用中利用 AI 模型的过程。此仓库中开源的核心 ModelScope 库提供了接口和实现，允许开发者执行模型推理、训练和评估。
...
嵌入模型通常用于检索增强生成（RAG）流程，既作为数据索引的一部分，也用于后续检索。有关更详细的说明，请参阅我们的 [RAG 教程](/oss/python/langchain/rag)。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\embeddings\modelscope_embedding.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
