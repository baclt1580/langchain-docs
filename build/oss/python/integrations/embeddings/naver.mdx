---
title: "Naver 集成"
description: "使用 LangChain Python 与 Naver 嵌入模型进行集成。"
---

本笔记本介绍了如何使用 CLOVA Studio 提供的嵌入模型入门。有关 `ClovaXEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain#%EC%9E%84%EB%B2%A0%EB%94%A9%EB%8F%84%EA%B5%AC%EC%9D%B4%EC%9A%A9)。
...
3. 创建一个要使用的 CLOVA Studio 测试应用或服务应用（参见 [CLOVA Studio 测试应用设置](https://guide.ncloud-docs.com/docs/clovastudio-explorer03#%ED%85%8C%EC%8A%A4%ED%8A%B8%EC%95%B1%EC%83%9D%EC%84%B1).）
4. 颁发测试或服务 API 密钥（参见 [CLOVA Studio API 密钥指南](https://guide.ncloud-docs.com/docs/clovastudio-explorer-testapp).）
...
- CLOVA Studio 中有多种可用的嵌入模型。有关更多详细信息，请查看 [CLOVA Studio 嵌入 API 文档](https://guide.ncloud-docs.com/docs/en/clovastudio-explorer03#임베딩API)。
- 请注意，根据您的具体用例，您可能需要对嵌入结果进行归一化。
...
嵌入模型通常用于检索增强生成 (RAG) 流程，既作为数据索引的一部分，也用于后续的检索。有关更详细的说明，请参阅我们的 [RAG 教程](/oss/python/langchain/rag)。
