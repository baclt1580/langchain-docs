...
它通过将字符级解析器与 tokenizer 前缀树结合，仅允许包含导致潜在有效格式序列的字符的 token。
...
注意，Llama2 [需要批准才能访问模型](https://huggingface.co/meta-llama/Llama-2-7b-chat-hf)。
...
***结果通常更接近于架构定义的 JSON 对象，而不是符合该架构的 JSON 对象。让我们尝试强制正确的输出。***
...
这意味着，如果您需要为 API 调用或类似用途格式化 JSON，如果您可以生成架构（来自 pydantic 模型或通用），您可以使用此库确保 JSON 输出正确，最小化幻觉风险。
...
LMFormatEnforcer 有一种额外的模式，使用正则表达式来过滤输出。请注意，它在底层使用了 [interegular](https://pypi.org/project/interegular/)，因此不支持 100% 的正则表达式功能。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\lmformatenforcer_experimental.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
