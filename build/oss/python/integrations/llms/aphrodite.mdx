...
* 由 vLLM 提供的注意力机制，实现高吞吐量和低延迟
* 支持多种 SOTA 采样方法
* Exllamav2 GPTQ 内核，在较低批次大小下实现更好的吞吐量

本笔记本介绍了如何使用 LangChain 和 Aphrodite 来使用 LLM。

要使用它，您应该已安装 `aphrodite-engine` Python 包。
...
若要使用 LLM 类运行多 GPU 推理，请将 `tensor_parallel_size` 参数设置为您想要使用的 GPU 数量。例如，要在 4 个 GPU 上运行推理
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\aphrodite.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
