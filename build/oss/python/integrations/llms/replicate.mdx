...
>[Replicate](https://replicate.com/blog/machine-learning-needs-better-tools) 在云端运行机器学习模型。我们拥有一个开源模型库，你可以用几行代码运行它们。如果你正在构建自己的机器学习模型，Replicate 使得大规模部署变得容易。
...
要运行此笔记本，你需要创建一个 [replicate](https://replicate.com) 账户并安装 [replicate python client](https://github.com/replicate/replicate-python)。
...
在 [replicate explore page](https://replicate.com/explore) 上查找一个模型，然后将模型名称和版本以 `model_name/version` 格式粘贴进去。
...
作为另一个例子，对于此 [dolly model](https://replicate.com/replicate/dolly-v2-12b)，点击 API 标签页。模型名称/版本将是：`replicate/dolly-v2-12b:ef0e1aefc61f8e096ebe4db6b2bacc297daf2ef6899f0f7e001ec445893500e5`
...
你可以选择流式传输生成的响应，这对于向用户展示耗时生成过程的交互性很有帮助。有关更多信息，请参阅 [Streaming](/oss/python/langchain/streaming) 的详细文档。
...
你也可以指定停止序列。如果你有一个确定的停止序列用于解析生成结果，那么一旦达到一个或多个停止序列就取消生成会更好（更便宜、更快！），而不是让模型一直说到指定的 `max_length`。无论是否处于流式模式，停止序列都有效，并且 Replicate 仅对直到停止序列的生成部分收费。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\replicate.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
