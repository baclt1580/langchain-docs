...
`Gradient` 允许通过简单的 Web API 对 LLM 进行微调并获得补全结果。

本笔记本介绍如何使用 LangChain 与 [Gradient](https://gradient.ai/) 配合。
...
由于我们稍后想要微调我们的模型，我们选择 ID 为 `674119b5-f19e-4856-add2-767ae7f7d7ef_model_adapter` 的 model_adapter，但您也可以使用任何基础或可微调的模型。
...
嗯——这是错的——旧金山 49 人队并没有赢。
该问题的正确答案应该是 `The Dallas Cowboys!`。

让我们通过使用 PromptTemplate 针对正确答案进行微调，来提高得到正确答案的几率。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\gradient.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
