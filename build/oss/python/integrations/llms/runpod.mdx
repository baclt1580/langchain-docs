...
本指南介绍如何使用 LangChain `RunPod` LLM 类来与托管在 [RunPod Serverless](https://www.runpod.io/serverless-gpu) 上的文本生成模型进行交互。
...
2. **部署 LLM 端点：** 遵循 [RunPod 提供者指南](/oss/python/integrations/providers/runpod#setup) 中的设置步骤，在 RunPod Serverless 上部署兼容的文本生成端点并获取其端点 ID。
...
- **流式传输：** 对于通过 `/stream` 端点进行的模拟流式传输，处理器必须在状态响应中填充 `"stream"` 键，包含一个块字典列表，如 `[{"output": "token1"}, {"output": "token2"}]`。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\runpod.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
