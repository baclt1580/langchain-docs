...
ModelScope ([Home](https://www.modelscope.cn/) | [GitHub](https://github.com/modelscope/modelscope)) 基于“模型即服务”（MaaS）的理念构建。它致力于汇聚 AI 社区中最先进的机器学习模型，并简化在现实应用中利用 AI 模型的过程。此仓库中开源的核心 ModelScope 库提供了接口和实现，允许开发者执行模型推理、训练和评估。这将帮助您使用 LangChain Python 快速上手 ModelScope 补全模型（LLM）。
...
前往 [ModelScope](https://modelscope.cn/) 注册 ModelScope 并生成一个 [SDK token](https://modelscope.cn/my/myaccesstoken)。完成此操作后，请设置 `MODELSCOPE_SDK_TOKEN` 环境变量：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\llms\modelscope_endpoint.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
