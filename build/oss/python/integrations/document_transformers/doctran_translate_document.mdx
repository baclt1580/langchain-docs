...
通过嵌入进行文档比较的优势在于能够跨多种语言工作。"Harrison says hello" 和 "Harrison dice hola" 将在向量空间中占据相似的位置，因为它们在语义上具有相同的含义。

然而，在向量化之前使用 LLM **将文档翻译成其他语言** 仍然是有用的。当用户预计会以不同语言查询知识库，或者对于特定语言没有可用的最先进嵌入模型时，这一点尤其有帮助。
...
翻译文档后，结果将作为新文档返回，其 `page_content` 已翻译为目标语言。当文档被分块成多个部分时，异步版本将提高性能。它还将确保以正确的顺序返回输出。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_transformers\doctran_translate_document.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
