...
> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收一个文档列表，并根据文档与查询的相关性对这些文档进行重新排序。与仅关注文档和查询语义相似性的嵌入方法相比，排序 API 可以为您提供文档对给定查询回答效果的精确分数。排序 API 可用于在检索到初始候选文档集后提升搜索结果的质量。

> 排序 API 是无状态的，因此在调用 API 前无需索引文档。您只需传入查询和文档即可。这使得该 API 非常适合对来自任何文档检索器的文档进行重排序。

> 更多信息，请参阅 [对文档进行排序和重排序](https://cloud.google.com/generative-ai-app-builder/docs/ranking)。
...
在此示例中，我们将使用 [Google 维基页面](https://en.wikipedia.org/wiki/Google) 来演示 Vertex Ranking API 的工作原理。
...
if vectordb is not None:  # 如果 vectordb 已存在，则删除它
...
# 创建 ContextualCompressionRetriever，将 VertexAIRanker 作为重排序器
...
排序 API 用于提升排序质量，并确定一个分数来指示每条记录与查询的相关性。

您可以看到未排序文档和已排序文档之间的差异。排序 API 将语义最相关的文档移动到 LLM 上下文窗口的顶部，从而帮助其形成更好的推理答案。
...
让我们检查几个重排序后的文档。我们观察到检索器仍然返回相关的 LangChain 类型 [文档](https://reference.langchain.com/python/langchain-core/documents/base/Document)，但作为元数据字段的一部分，我们还收到了来自 Ranking API 的 `relevance_score`。
...
这展示了一个完整的 RAG 链示例，其中包含一个简单的提示模板，演示了如何使用 Vertex Ranking API 执行重排序。
...
reranker = VertexAIRank(
    ...
    location_id=RANKING_LOCATION_ID,
    ranking_config="default_ranking_config",
    title_field="source",  # 来自您现有文档的元数据字段键
    top_n=5,
)
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_transformers\google_cloud_vertexai_rerank.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
