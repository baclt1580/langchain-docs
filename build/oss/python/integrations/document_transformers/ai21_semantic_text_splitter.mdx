...
此示例展示如何使用 AI21SemanticTextSplitter 根据语义将文本分割成块。
...
此示例展示如何使用 AI21SemanticTextSplitter 根据语义将文本分割成块，然后根据 `chunk_size` 合并这些块。
...
此示例展示如何使用 AI21SemanticTextSplitter 根据语义将文本分割成文档。每个文档的元数据将包含一个类型字段。
...
此示例展示如何使用 AI21SemanticTextSplitter 根据语义将文本分割成文档。每个文档的元数据将包含一个起始索引。
**注意**：起始索引提供的是块顺序的指示，而不是每个块的实际起始索引。
...
此示例展示如何使用 AI21SemanticTextSplitter 根据语义将文档列表分割成块。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_transformers\ai21_semantic_text_splitter.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
