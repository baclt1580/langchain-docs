...
>[Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/) 是一个用于解析 HTML 和 XML 文档的 Python 包（包括处理标记不良的情况，例如未闭合的标签，因此得名“标签汤”）。
>它为解析后的页面创建一个解析树，可用于从 HTML 中提取数据，[3] 这对于网络爬虫非常有用。

`Beautiful Soup` 提供了对 HTML 内容的精细控制，能够提取特定标签、移除内容以及清理 HTML。

它适用于需要根据具体需求提取特定信息并清理 HTML 内容的场景。

例如，我们可以从 HTML 内容中提取 `<p>、<li>、<div> 和 <a>` 标签内的文本内容：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_transformers\beautiful_soup.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
