---
title: "Apache AGE 集成"
description: "使用 LangChain Python 与 Apache AGE 图进行集成。"
---

>[Apache AGE](https://age.apache.org/) 是一个提供图数据库功能的 PostgreSQL 扩展。AGE 是 A Graph Extension 的缩写，灵感来源于 Bitnine 对 PostgreSQL 10 的分支 AgensGraph，这是一个多模型数据库。该项目的目标是创建单一存储，能够同时处理关系型和图模型数据，以便用户可以使用标准 ANSI SQL 以及 openCypher（图查询语言）。`Apache AGE` 存储的数据元素包括节点、连接它们的边，以及节点和边的属性。

>本笔记本展示了如何使用 LLM 为图数据库提供自然语言接口，您可以使用 `Cypher` 查询语言对其进行查询。

>[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式图查询语言，允许在属性图中进行表达力强且高效的数据查询。

## 设置

您需要拥有一个已运行并安装了 AGE 扩展的 `Postgre` 实例。测试的一个选项是使用官方 AGE docker 镜像运行 docker 容器。
...
有关在 Docker 中运行的其他说明，请参阅 [Apache AGE Docker Hub 页面](https://hub.docker.com/r/apache/age)。
...
假设您的数据库为空，您可以使用 Cypher 查询语言来填充它。以下 Cypher 语句是可幂等的，这意味着无论运行一次还是多次，数据库信息都将相同。
...
我们现在可以使用图 cypher QA 链来询问关于图的问题
...
您可以定义对于特定问题希望 LLM 生成的 Cypher 语句
...
您可以使用 `cypher_llm` 和 `qa_llm` 参数来定义不同的 llms
...
您可以使用 `include_types` 或 `exclude_types` 在生成 Cypher 语句时忽略图架构的一部分。
...
您可以使用 `validate_cypher` 参数来验证并更正生成的 Cypher 语句中的关系方向
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\graphs\apache_age.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
