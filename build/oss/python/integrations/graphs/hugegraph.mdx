...
>[HugeGraph](https://hugegraph.apache.org/) 是一个方便、高效且适应性强的图数据库，兼容
>the `Apache TinkerPop3` 框架和 `Gremlin` 查询语言。
>
>[Gremlin](https://en.wikipedia.org/wiki/Gremlin_(query_language)) 是 `Apache Software Foundation` 的 `Apache TinkerPop` 开发的图遍历语言和虚拟机。
...
如果我们要在应用程序中连接 HugeGraph，我们需要安装 Python SDK：

```
pip3 install hugegraph-python
```

如果你使用的是 Docker 容器，你需要等待几秒钟让数据库启动，然后我们需要为数据库创建模式并写入图数据。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\graphs\hugegraph.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
