...
Memgraph 是一个开源图数据库，专为动态分析环境优化，并与 Neo4j 兼容。要查询该数据库，Memgraph 使用 Cypher —— 属性图数据库中最广泛采用、完全规范且开放的查询语言。
...
要浏览本指南，您需要安装 [Docker](https://www.docker.com/get-started/) 和 [Python 3.x](https://www.python.org/)。

为了首次快速运行 **Memgraph Platform**（Memgraph 数据库 + MAGE 库 + Memgraph Lab），请执行以下操作：
...
两个命令都会运行一个脚本，该脚本将 Docker Compose 文件下载到您的系统，并在两个单独的容器中构建并启动 `memgraph-mage` 和 `memgraph-lab` Docker 服务。现在您已成功启动并运行 Memgraph！有关安装过程的更多信息，请参阅 [Memgraph 文档](https://memgraph.com/docs/getting-started/install-memgraph)。

要使用 LangChain，请安装并导入所有必要的包。我们将使用包管理器 [pip](https://pip.pypa.io/en/stable/installation/)，并配合 `--user` 标志以确保正确的权限。如果您已安装 Python 3.4 或更高版本，则默认包含 `pip`。您可以使用以下命令安装所有所需的包：

```
pip install langchain langchain-openai langchain-memgraph --user
```

您可以在此笔记本中运行提供的代码块，也可以使用单独的 Python 文件来实验 Memgraph 和 LangChain。

## 使用自然语言查询

Memgraph 与 LangChain 的集成包括自然语言查询功能。要利用它，首先进行所有必要的导入。我们将随代码出现讨论它们。

首先，实例化 `MemgraphGraph`。此对象保存与正在运行的 Memgraph 实例的连接。请确保正确设置所有环境变量。
...
`refresh_schema` 初始设置为 `False`，因为数据库中还没有数据，我们想避免不必要的数据库调用。

### 填充数据库

要填充数据库，首先确保它是空的。最有效的方法是切换到内存分析存储模式，删除图，然后返回到内存事务模式。了解有关 Memgraph [存储模式](https://memgraph.com/docs/fundamentals/storage-memory-usage#storage-modes) 的更多信息。

我们要添加到数据库的数据是关于不同流派的游戏，这些游戏可在各种平台上获得，并与出版商相关。
...
注意 `graph` 对象如何持有 `query` 方法。该方法在 Memgraph 中执行查询，并且也被 `MemgraphQAChain` 用于查询数据库。

### 刷新图模式

由于在 Memgraph 中创建了新数据，因此需要刷新模式。生成的模式将由 `MemgraphQAChain` 用于指导 LLM 更好地生成 Cypher 查询。
...
要与 OpenAI API 交互，必须将您的 API 密钥配置为环境变量。这确保了您请求的正确授权。您可以在 [OpenAI 帮助中心](https://help.openai.com/en/articles/4936850-where-do-i-find-my-secret-api-key) 找到更多关于获取 API 密钥的信息。要配置 API 密钥，您可以使用 Python [os](https://docs.python.org/3/library/os.html) 包：
...
接下来，创建 `MemgraphQAChain`，它将在基于您的图数据进行问答过程中被利用。`temperature` 参数设置为零，以确保可预测和一致的答案。您可以将 `verbose` 参数设置为 `True` 以接收有关查询生成的更详细消息。
...
要修改链的行为并获得更多上下文或附加信息，您可以修改链的参数。

##### 直接返回查询结果

`return_direct` 修饰符指定是返回执行的 Cypher 查询的直接结果还是处理后的自然语言响应。
...
`return_intermediate_steps` 链修饰符通过除了初始查询结果外还包括查询的中间步骤来增强返回的响应。
...
当您想要限制最大查询结果数量时，可以使用 `top_k` 修饰符。
...
随着解决方案复杂度的增加，您可能会遇到需要仔细处理的不同用例。确保应用程序的可扩展性对于维持顺畅的用户流程至关重要，没有任何阻碍。

让我们再次实例化我们的链，并尝试问一些用户可能会问的问题。
...
生成的 Cypher 查询看起来没问题，但我们没有收到任何响应信息。这说明了在使用 LLM 时的一个常见挑战——用户措辞查询的方式与数据存储方式之间的不匹配。在这种情况下，用户感知与实际数据存储之间的差异可能导致不匹配。提示优化（prompt refinement）是解决此问题的有效方案，即磨练模型的提示以更好地理解这些区别。通过提示优化，模型能够更熟练地生成精确且相关的查询，从而成功检索所需数据。

#### 提示优化

为了解决这个问题，我们可以调整 QA 链的初始 Cypher 提示。这涉及向 LLM 添加指导，说明用户如何引用特定平台，例如我们案例中的 PS5。我们使用 LangChain PromptTemplate 实现这一点，创建一个修改后的初始提示。然后将此修改后的提示作为参数提供给我们的改进 `MemgraphQAChain` 实例。
...
现在，通过使用包含平台命名指导的修订版初始 Cypher 提示，我们正在获得准确且相关的结果，这些结果与用户查询更加一致。

这种方法允许进一步改进您的 QA 链。您可以轻松地将额外的提示优化数据集成到您的链中，从而提高应用程序的整体用户体验。

## 构建知识图谱

将非结构化数据转换为结构化数据并非一项容易或简单的任务。本指南将展示如何利用 LLM 帮助我们做到这一点，以及如何在 Memgraph 中构建知识图谱。创建知识图谱后，您可以将其用于 GraphRAG 应用程序。

从文本构建知识图谱的步骤如下：

- [从文本中提取结构化信息](#extracting-structured-information-from-text)：LLM 用于从文本中提取结构化图信息，形式为节点和关系。
- [存储到 Memgraph](#storing-into-memgraph)：将提取的结构化图信息存储到 Memgraph 中。

### 从文本中提取结构化信息

除了 [设置部分](#setting-up) 中的所有导入外，还要导入 `LLMGraphTransformer` 和 [`Document`]，它们将用于从文本中提取结构化信息。
...
下面是关于查尔斯·达尔文 ([source](https://en.wikipedia.org/wiki/Charles_Darwin)) 的示例文本，将从中构建知识图谱。
...
在底层，LLM 从文本中提取重要实体并以节点和关系的列表形式返回它们。以下是其外观：
...
一旦您准备好以 `GraphDocument` 格式的数据，即节点和关系，您就可以使用 `add_graph_documents` 方法将其导入 Memgraph。该方法将 `graph_documents` 列表转换为需要在 Memgraph 中执行的适当 Cypher 查询。完成后，知识图谱就存储在 Memgraph 中。
...
<Note>
图构建过程是非确定性的，因为用于从无结构化数据生成节点和关系的 LLM 是非确定性的。
</Note>

### 其他选项

此外，您可以根据需求灵活定义要提取的特定类型的节点和关系。
...
您的图也可以在所有节点上具有 `__Entity__` 标签，这将用于索引以实现更快的检索。
...
还有一个选项可以包含图中获取的信息源。为此，将 `include_source` 设置为 `True`，然后存储源文档，并使用 `MENTIONS` 关系将其链接到图中的节点。
...
注意源的内容是如何存储的，并且生成了 `id` 属性，因为文档没有任何 `id`。
您可以同时拥有 `__Entity__` 标签和文档源。但是，请注意两者都占用内存，特别是包含源时，因为内容字符串很长。

最后，您可以像前一节解释的那样查询知识图谱：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\graphs\memgraph.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
