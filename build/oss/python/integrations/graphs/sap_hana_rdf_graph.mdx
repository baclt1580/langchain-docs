...
详细步骤，请参阅：[启用三元组存储](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/enable-triple-store/)
...
* **`graph_uri`**: 包含您 RDF 数据的命名图（或 `"DEFAULT"`）
* **以下之一**：
  1. **`ontology_query`**: 用于提取架构三元组的 SPARQL CONSTRUCT
...
* **`graph_uri`**:
  您 SAP HANA Cloud 实例中包含实例数据的命名图（有时超过 10 万条三元组）。
  如果提供 `None`、`""` 或 `"DEFAULT"`，则使用默认图。
* **本体**: 精简的架构（通常为 ~50-100 条三元组），描述类、属性、域、范围、标签、注释和子类关系。本体指导 SPARQL 生成和结果解释。

### 使用 **DEFAULT** 图创建图实例

有关 DEFAULT 图的更多信息，请查看 [DEFAULT 图和命名图](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/default-graph-and-named-graphs)。
...
(`ontology_local_file` + `ontology_local_file_format`): 从本地 RDF 本体文件加载架构。
...
(`auto_extract_ontology=True`): 直接从您的实例数据推断架构信息。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\graphs\sap_hana_rdf_graph.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
