...
description: "使用 LangChain Python 与 OceanbaseVectorStore 进行集成。"
---
...
检查与 OceanBase 的连接并设置向量数据的内存使用率：
...
配置嵌入模型的 API 密钥。此处我们以 DashScopeEmbeddings 为例。当如上所述使用 Docker 镜像部署 Oceanbase 时，只需按照以下脚本设置 host、port、user、password 和数据库名称。对于其他部署方法，请根据实际情况设置这些参数。
...
一旦您的向量存储已创建并添加了相关文档，您很可能希望在运行链或代理期间查询它。
...
results = vector_store.similarity_search_with_score(
    query="thud", k=1, filter={"source": "example.com"}
)
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\oceanbase.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
