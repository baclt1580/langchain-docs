...
> [YDB](https://ydb.tech/) 是一个多功能的开源分布式 SQL 数据库，它将高可用性和可扩展性与强一致性和 ACID 事务相结合。它同时支持事务性（OLTP）、分析性（OLAP）和流式工作负载。
...
本笔记本没有凭据，只需确保你已如上所示安装了包即可。

如果你希望获得最佳级别的模型调用自动追踪，你也可以通过取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
你可以使用 `add_documents` 函数将项添加到你的向量存储中。
...
你可以使用 `delete` 函数通过 ID 从向量存储中删除项。
...
一旦创建了向量存储并添加了相关文档，在执行链或代理时，你很可能会想要查询它。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\ydb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
