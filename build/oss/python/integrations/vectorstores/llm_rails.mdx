...
> [LLMRails](https://www.llmrails.com/) 是一个用于构建 GenAI 应用的 API 平台。它提供了一个易于使用的 API，用于文档索引和查询，由 LLMRails 管理，并针对性能和准确性进行了优化。
...
本笔记本展示了如何使用与 `LLMRails` 的 langchain 集成功能相关的内容。
请注意，与此类别中的许多其他集成不同，LLMRails 为检索增强生成提供了端到端托管服务，包括：

1. 一种从文档文件中提取文本并将其拆分为句子的方法。
...
您将需要一个 LLMRails 账户才能将 LLMRails 与 LangChain 一起使用。要开始使用，请执行以下步骤：

1. [注册](https://console.llmrails.com/signup) 一个 LLMRails 账户，如果您还没有的话。
2. 接下来您需要创建 API 密钥以访问 API。点击语料库视图中的 **"API 密钥"** 标签页，然后点击 **"创建 API 密钥"** 按钮。为您的密钥命名。点击“创建密钥”，您现在拥有一个活动的 API 密钥。请保密此密钥。

要将 LangChain 与 LLMRails 一起使用，您需要拥有此值：api_key。
您可以通过以下两种方式将这些提供给 LangChain：
...
要向数据存储添加文本，首先必须前往 [Datastores](https://console.llmrails.com/datastores) 页面并创建一个。点击 Create Datastore 按钮并为您的数据存储选择名称和嵌入模型。然后从新创建的数据存储设置中获取您的数据存储 id。
...
有时我们可能希望执行搜索，但也获得相关性分数，以了解特定结果有多好。
...
LLMRails 与其他所有 LangChain 向量存储一样，最常用作 LangChain Retriever：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\llm_rails.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
