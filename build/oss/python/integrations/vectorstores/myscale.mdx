...
>[MyScale](https://docs.myscale.com/en/overview/) 是一个基于云的数据库，专为 AI 应用和解决方案优化，构建于开源的 [ClickHouse](https://github.com/ClickHouse/ClickHouse) 之上。
...
1. 环境变量
    ...
    您可以在我们的 SaaS 上轻松找到您的账户、密码和其他信息。详细信息请参阅 [this document](https://docs.myscale.com/en/cluster-management/)

    所有 `MyScaleSettings` 下的属性都可以使用前缀 `MYSCALE_` 设置，且不区分大小写。
...
如果您在设置中自定义了 `column_map`，则可以使用如下过滤器进行搜索：
...
您可以使用 `.drop()` 方法删除表，或者使用 `.delete()` 方法部分删除数据。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\myscale.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
