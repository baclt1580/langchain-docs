...
[Kinetica](https://www.kinetica.com/) 是一个提供向量相似度搜索集成支持的数据库。
...
这需要 Kinetica 的一个实例，可以使用此处提供的说明轻松配置 - [安装说明](https://www.kinetica.com/developer-edition/)。
...
Kinetica 模块将尝试创建一个名为集合名称的表。请确保集合名称是唯一的，并且用户具有创建表的权限。
...
上面，我们从头创建了一个向量存储。但是，我们经常希望与现有的向量存储一起工作。为此，我们可以直接初始化它。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\kinetica.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
