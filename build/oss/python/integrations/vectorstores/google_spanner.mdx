...
本笔记本介绍如何使用 `Spanner` 配合 `SpannerVectorStore` 类进行向量搜索。
...
在 [Spanner 实例页面](https://console.cloud.google.com/spanner?_ga=2.223735448.2062268965.1707700487-2088871159.1707257687) 找到您的数据库值。
...
辅助方法 `init_vector_store_table()` 可用于为您创建一个具有正确模式的表。
...
要初始化 `SpannerVectorStore` 类，您需要提供 4 个必需参数，其他参数是可选的，仅在不同于默认值时才需要传递
...
要从向量存储中删除文档，请使用初始化 VectorStore 时对应于 `row_id`` 列值的 ID。
...
要从向量存储中删除文档，您可以直接使用文档本身。VectorStore 初始化期间提供的 content 列和 metadata 列将用于查找对应于文档的行。然后删除任何匹配的行。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\google_spanner.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
