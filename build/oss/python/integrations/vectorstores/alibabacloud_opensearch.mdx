...
description: "使用 LangChain Python 集成阿里云 OpenSearch 向量存储。"
---

>[阿里云 OpenSearch](https://www.alibabacloud.com/product/opensearch) 是一个开发智能搜索服务的一站式平台。`OpenSearch` 基于 `Alibaba` 开发的大规模分布式搜索引擎构建。`OpenSearch` 服务于阿里巴巴集团超过 500 个业务案例以及数千名阿里云客户。`OpenSearch` 帮助开发不同搜索场景下的搜索服务，包括电子商务、O2O、多媒体、内容产业、社区和论坛，以及企业大数据查询。

>`OpenSearch` 帮助您开发高质量、免维护且高性能的智能搜索服务，为用户提供高效的搜索效率和准确性。

>`OpenSearch` 提供向量搜索功能。在特定场景下，特别是试题搜索和图片搜索场景，您可以将向量搜索功能与多模态搜索功能结合使用，以提高搜索结果的准确性。

本笔记本展示了如何使用与 `Alibaba Cloud OpenSearch Vector Search Edition` 相关的功能。
...
要运行，您应该拥有一个已启动运行的 [OpenSearch 向量搜索版](https://opensearch.console.aliyun.com) 实例。
...
如果您在使用过程中遇到任何问题，请随时联系 [xingshaomin.xsm@alibaba-inc.com](mailto:xingshaomin.xsm@alibaba-inc.com)，我们将尽力为您提供协助和支持。
...
我们要使用 `OpenAIEmbeddings`，因此需要获取 OpenAI API Key。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\alibabacloud_opensearch.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
