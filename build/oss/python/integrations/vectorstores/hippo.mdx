...
>[Transwarp Hippo](https://www.transwarp.cn/en/subproduct/hippo) 是一款企业级云原生分布式向量数据库，支持海量基于向量的数据集的存储、检索和管理。它高效解决了向量相似度搜索和高密度向量聚类等问题。`Hippo`具有高可用性、高性能和易于扩展的特点。它拥有多种功能，如多种向量搜索索引、数据分区和分片、数据持久化、增量数据摄入、向量标量字段过滤以及混合查询。它能有效满足企业对海量向量数据的高实时搜索需求

## 入门指南

这里唯一的先决条件是从 OpenAI 网站获取的 API 密钥。请确保您已经启动了一个 Hippo 实例。

## 安装依赖项

首先，我们需要安装某些依赖项，例如 OpenAI、LangChain 和 Hippo-API。请注意，您应该安装适合您环境的相应版本。
...
在此，我们使用 LangChain 的 CharacterTextSplitter 进行分割。分隔符是句号。分割后，文本段不超过 1000 个字符，重复字符数为 0。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\hippo.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
