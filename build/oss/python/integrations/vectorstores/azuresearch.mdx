...
[Azure AI 搜索](https://learn.microsoft.com/azure/search/search-what-is-azure-search)（以前称为 `Azure Search` 和 `Azure Cognitive Search`）是一项云搜索服务，为开发人员提供基础设施、API 和工具，用于大规模检索向量、关键词和混合查询的信息。

您需要安装 `langchain-community` 并使用 `pip install -qU langchain-community` 来使用此集成
...
[`OpenAIEmbeddings`](https://reference.langchain.com/python/langchain-openai/embeddings/base/OpenAIEmbeddings) 是假设使用的，但如果您使用的是 Azure OpenAI，请改用 [`AzureOpenAIEmbeddings`](https://reference.langchain.com/python/langchain-openai/embeddings/azure/AzureOpenAIEmbeddings)。
...
设置您的 OpenAI 提供商变量。您需要一个 [OpenAI 账户](https://platform.openai.com/docs/quickstart?context=python) 或一个 [Azure OpenAI 账户](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource) 来生成嵌入。
...
您需要一个 [Azure 订阅](https://azure.microsoft.com/en-us/free/search) 和一个 [Azure AI Search 服务](https://learn.microsoft.com/azure/search/search-create-service-portal) 才能使用此向量存储集成。小规模和工作负载有限的场景有免费版本可用。

设置您的 Azure AI Search URL 和管理员 API 密钥变量。您可以从 [Azure 门户](https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices) 获取这些变量。
...
创建 [`OpenAIEmbeddings`](https://reference.langchain.com/python/langchain-openai/embeddings/base/OpenAIEmbeddings) 和 AzureSearch 类的实例。完成此步骤后，您应该在 Azure AI Search 资源上拥有一个空的搜索索引。集成模块提供默认架构。
...
此架构显示字段定义。它是默认架构加上几个新字段，这些字段被标记为可过滤。因为它使用默认向量配置，所以这里不会看到向量配置或向量配置文件覆盖。默认向量配置文件的名称是 "myHnswProfile"，它使用分层可导航小世界 (HNSW) 向量配置对 content_vector 字段进行索引和查询。

此步骤中没有该架构的数据。当您执行单元格时，您应该在 Azure AI Search 上获得一个空索引。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\vectorstores\azuresearch.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
