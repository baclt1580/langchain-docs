...
> [Firestore](https://cloud.google.com/firestore) 是一个无服务器文档导向数据库，可根据需求灵活扩展。借助 Firestore 的 LangChain 集成，扩展您的数据库应用以构建 AI 驱动的体验。
...
确认在此笔记本的运行时环境中可以访问数据库后，填写以下值并在运行示例脚本之前执行该单元格。
...
该集成位于独立的 `langchain-google-firestore` 包中，因此我们需要安装它。
...
1. `source` - 一个 Query、CollectionGroup、DocumentReference 实例，或指向 Firestore 集合的单个以 `\` 分隔的路径。
...
当 `page_content` 仅包含一个字段时，信息将仅为字段值。否则，`page_content` 将以 JSON 格式呈现。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\google_firestore.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
