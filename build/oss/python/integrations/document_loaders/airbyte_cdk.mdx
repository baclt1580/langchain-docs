...
>[Airbyte](https://github.com/airbytehq/airbyte) 是一个用于 ELT 管道的数据集成平台，可将来自 API、数据库和文件的数据传输到数据仓库和湖泊。它拥有最大的 ELT 连接器目录，支持多种数据仓库和数据库。
...
然后，你可以从 [Airbyte GitHub 仓库](https://github.com/airbytehq/airbyte/tree/master/airbyte-integrations/connectors) 安装现有的连接器，或使用 [Airbyte CDK](https://docs.airbyte.io/connector-development/connector-development) 创建自己的连接器。
...
现在你可以基于导入的源创建一个 `AirbyteCDKLoader`。它接受一个传递给连接器的 `config` 对象。你还需要通过名称（`stream_name`）选择要从中检索记录的流。有关配置对象和可用流的更多信息，请查看连接器的文档页面和规范定义。对于 GitHub 连接器，这些是：
...
由于 `load` 返回一个列表，它会阻塞直到所有文档加载完成。为了更好地控制这个过程，你也可以使用 `lazy_load` 方法，它返回一个迭代器：
...
某些流允许增量加载，这意味着源会跟踪已同步的记录，不会再次加载它们。这对于数据量大且频繁更新的源非常有用。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\airbyte_cdk.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
