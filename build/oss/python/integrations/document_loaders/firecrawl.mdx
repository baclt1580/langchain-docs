...
[FireCrawl](https://firecrawl.dev/?ref=langchain) 能够抓取任何网站并将其转换为可供大语言模型使用的数据。它会抓取所有可访问的子页面，并为每个页面提供干净的 Markdown 和元数据。无需站点地图。

FireCrawl 处理复杂的任务，如反向代理、缓存、速率限制以及被 JavaScript 屏蔽的内容。由 [mendable.ai](https://mendable.ai) 团队开发。
...
- `scrape`：抓取单个 URL 并返回其 Markdown。
- `crawl`：抓取该 URL 及其所有可访问的子页面，并返回每个页面的 Markdown。
- `map`：映射 URL 并返回一个语义相关页面的列表。
...
你也可以向加载器传递 `params`。这是一个传递给抓取器的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
...
你也可以向加载器传递 `params`。这是一个传递给加载器的选项字典。更多信息请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。

---

## API 参考

有关 `FireCrawlLoader` 所有功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/langchain-community/document_loaders/firecrawl/FireCrawlLoader)

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\firecrawl.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
