...
>[Etherscan](https://docs.etherscan.io/) 是以太坊（一个去中心化智能合约平台）领先的区块链浏览器、搜索、API 和分析平台。

## 概述

`Etherscan` 加载器使用 `Etherscan API` 来加载以太坊主网上特定账户下的交易历史。

您需要一个 `Etherscan API` 密钥才能继续。免费 `API` 密钥每秒有 5 次调用的配额。
...
* 检索以太坊主网上特定账户的以太币余额（以 Wei 为单位）

如果账户没有相应的交易，加载器将返回包含一个文档的列表。文档的内容为 ''。

您可以向加载器传递不同的过滤器以访问上述提到的不同功能：
...
所有与交易历史相关的功能都受限于 Etherscan 的最大 1000 条历史记录限制。您可以使用以下参数查找您需要的交易历史：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\etherscan.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
