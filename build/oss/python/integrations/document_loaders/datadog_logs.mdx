...
该加载器使用 `datadog_api_client` Python 包从 Datadog 中获取应用程序的日志。您必须使用您的 `Datadog API 密钥` 和 `APP 密钥` 初始化加载器，并传入查询以提取所需的日志。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\datadog_logs.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
