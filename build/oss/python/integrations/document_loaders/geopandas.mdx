...
几何运算由 shapely 执行。Geopandas 进一步依赖 fiona 进行文件访问，依赖 matplotlib 进行绘图。
...
以 [`Open City Data`](/oss/python/integrations/document_loaders/open_city_data) 为例输入，创建一个 GeoPandas 数据框。
...
将 GeoPandas 数据框加载为 [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) 以供下游处理（嵌入、聊天等）。

默认情况下，`geometry` 列将作为 `page_content`，所有其他列将放入 `metadata` 中。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\geopandas.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
