...
它专为解析学术论文而设计，在此类任务上表现尤为出色。注意：如果提供给 Grobid 的文章是大型文档（例如学位论文）且超过一定元素数量，可能无法被处理。

该加载器使用 Grobid 将 PDF 解析为保留文本段落相关元数据的 `Document` 对象。

---
最佳方式是通过 Docker 安装 Grobid，详见 [grobid.readthedocs.io/en/latest/Grobid-docker/](https://grobid.readthedocs.io/en/latest/Grobid-docker/)。
更多说明请参阅 [Grobid 供应商页面](/oss/python/integrations/providers/grobid)。

当 Grobid 正常运行后，您可以按以下方式操作。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\grobid.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
