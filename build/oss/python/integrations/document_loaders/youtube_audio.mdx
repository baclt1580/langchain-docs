...
我们将使用 `OpenAIWhisperParser`，它会调用 OpenAI Whisper API 将音频转录为文本，
以及 `OpenAIWhisperParserLocal` 用于本地支持，可在私有云或本地环境中运行。
...
我们以 Andrej Karpathy YouTube 课程的第一讲为例！
...
有了 `Document` 对象，我们可以轻松启用聊天/问答功能。
...
```python
query = "对于任何 token，x、k、v 和 q 分别是什么？"
qa_chain.run(query)
```
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\document_loaders\youtube_audio.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
