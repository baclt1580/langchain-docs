...
description: "使用 LangChain Python 与 Valyucontext 工具进行集成。"
---

>[Valyu](https://www.valyu.network/) 允许 AI 应用程序和代理搜索互联网和专有数据源，以获取适合 LLM 的相关信息。

本笔记本介绍如何在 LangChain 中使用 Valyu 上下文工具。

首先，获取一个 Valyu API 密钥并将其添加为环境变量。通过 [在此处注册](https://platform.valyu.network/) 可获得 10 美元免费额度。
...
此处展示如何实例化 Valyu 搜索工具的实例。此工具允许您使用 Valyu 的 Context API 端点完成搜索查询。
...
我们可以通过将工具绑定到代理来直接将我们的工具与代理执行器一起使用。这使代理能够动态设置 Valyu 搜索工具的可用参数。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\valyu_search.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
