...
[Bright Data](https://brightdata.com/) 提供强大的 SERP API，允许您使用地理定位和高级自定义选项查询搜索引擎（Google、Bing、DuckDuckGo、Yandex），特别适用于需要实时网络信息的 AI 代理。
...
此处展示如何实例化 BrightDataSERP 工具的实例。此工具允许您使用 Bright Data 的 SERP API 执行搜索引擎查询，具有各种自定义选项，包括地理定位、语言偏好、设备类型模拟以及特定搜索类型。

该工具在实例化期间接受各种参数：

- `bright_data_api_key`（必需，str）：用于认证的 Bright Data API 密钥。
...
- `country`（可选，str）：本地化搜索结果的两字母国家代码（例如 "us"、"gb"、"de"、"jp"）。默认为 "us"。
- `language`（可选，str）：搜索结果的两字母语言代码（例如 "en"、"es"、"fr"、"de"）。默认为 "en"。
...
- `search_type`（可选，str）：要执行的搜索类型。选项包括：
  ...
  - "nws"：新闻搜索
  - "jobs"：职位搜索
- `device_type`（可选，str）：要模拟的设备类型。选项包括：
  - None（默认）：桌面设备
  - "mobile"：通用移动设备
  - "ios"：iOS 设备（iPhone）
  - "android"：Android 设备
...
|`country`|str|本地化结果的两字母国家代码（默认："us"）|
|`language`|str|两字母语言代码（默认："en"）|
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\brightdata_serp.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
