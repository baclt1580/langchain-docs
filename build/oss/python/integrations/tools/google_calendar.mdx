...
> [Google 日历](https://workspace.google.com/intl/en-419/products/calendar/) 是 Google Workspace 的一款产品，允许用户组织日程和事件。它是一个基于云的日历，允许用户创建、编辑和删除事件。它还允许用户与他人共享日历。

## 概述

本笔记本将帮助您开始使用 Google 日历工具包。此工具包与 Google 日历 API 交互，以执行日历上的各种操作。它允许您：
...
要为各个工具启用自动追踪，请设置您的 [LangSmith](/langsmith/home) API 密钥：
...
在幕后，使用以下方法创建 `googleapi` 资源。您可以手动构建 `googleapi` 资源以获得更多的身份验证控制。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\google_calendar.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
