...
> [AskNews](https://asknews.app) 通过单个自然语言查询，将最新的全球新闻（或历史新闻）注入任何 LLM。具体来说，AskNews 每天通过翻译、总结、提取实体并将它们索引到热和冷向量数据库中，来丰富超过 30 万篇文章。AskNews 将这些向量数据库部署在低延迟端点上供您使用。当您查询 AskNews 时，您会得到一个经过提示优化的字符串，其中包含所有最相关的增强信息（例如实体、分类、翻译、总结）。这意味着您不需要管理自己的新闻 RAG，也不需要担心如何以简洁的方式向您的 LLM 传达新闻信息。
> AskNews 也致力于透明度，这就是为什么我们的覆盖范围在数百个国家、13 种语言和 5 万个来源中进行监控和多样化。如果您想跟踪我们的来源覆盖情况，可以访问我们的 [透明度仪表板](https://asknews.app/en/transparency)。
...
我们还需要设置我们的 AskNews API 凭据，这些可以在 [AskNews 控制台](https://my.asknews.app) 获取。
...
我们在这里展示如何将其作为智能体的一部分使用。我们使用 OpenAI Functions 智能体，因此我们需要为此设置并安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来获取提示词，因此我们需要安装它。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\asknews.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
