...
> [Bing Search](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/) 是一项 Azure 服务，可提供安全、无广告、基于位置感知的搜索结果，从数十亿网络文档中呈现相关信息。通过利用 Bing 通过单次 API 调用搜索数十亿网页、图片、视频和新闻的能力，帮助您的用户在全球网络上找到他们正在寻找的内容。

## 设置

按照 [instruction](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/create-bing-search-service-resource) 创建 Azure Bing Search v7 服务，并获取订阅密钥
...
我们在此展示如何将其作为 [agent](/oss/python/langchain/agents) 的一部分使用。我们使用 OpenAI Functions Agent，因此我们需要为此设置并安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来获取提示词，因此我们需要安装它。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\bing_search.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
