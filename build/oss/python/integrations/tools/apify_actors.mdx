...
>[Apify Actor](https://docs.apify.com/platform/actors) 是专为各种网页抓取、爬取和数据提取任务设计的云程序。这些 Actor 促进了从网络自动收集数据，使用户能够高效地提取、处理和存储信息。Actor 可用于执行诸如抓取电子商务网站的产品详情、监控价格变化或收集搜索引擎结果等任务。它们与 [Apify 数据集](https://docs.apify.com/platform/storage/dataset) 无缝集成，允许将 Actor 收集的 structured 数据存储、管理和导出为 JSON、CSV 或 Excel 等格式，以便进一步分析或使用。

## 概述

本笔记本将引导您使用 [Apify Actor](https://docs.apify.com/platform/actors) 与 LangChain 配合进行网页抓取和数据提取自动化。`langchain-apify` 包将 Apify 的基于云的工具与 LangChain 代理集成，实现 AI 应用的高效数据收集和数据处理。
...
此集成位于 [langchain-apify](https://pypi.org/project/langchain-apify/) 包中。该包可以使用 pip 安装。
...
此处我们实例化 `ApifyActorsTool` 以调用 [RAG Web Browser](https://apify.com/apify/rag-web-browser) Apify Actor。该 Actor 为 AI 和 LLM 应用提供网页浏览功能，类似于 ChatGPT 中的网页浏览功能。来自 [Apify Store](https://apify.com/store) 的任何 Actor 都可以这样使用。
...
`ApifyActorsTool` 接受单个参数，即 `run_input` —— 一个作为运行输入传递给 Actor 的字典。运行输入架构文档可在 Actor 详细信息页面的输入部分找到。参见 [RAG Web Browser 输入架构](https://apify.com/apify/rag-web-browser/input-schema)。
...
我们可以将创建的代理提供给 [agent](https://python.langchain.com/docs/tutorials/agents/)。当被要求搜索信息时，代理将调用 Apify Actor，后者将搜索网络，然后检索搜索结果。
...
浏览 [Apify Store](https://apify.com/store) 以发现更多适用于您用例的 Actor。
...
有关如何使用此集成的更多信息，请查看 [git 仓库](https://github.com/apify/langchain-apify) 或 [Apify 集成文档](https://docs.apify.com/platform/integrations/langgraph)。
...
[Apify MCP (模型上下文协议) 服务器](https://mcp.apify.com) 可以帮助您通过模型上下文协议发现可用 Actor、探索其输入架构并理解参数要求。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\apify_actors.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
