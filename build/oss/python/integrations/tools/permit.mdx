...
Permit 是一个访问控制平台，提供细粒度、实时的权限管理，支持多种模型，如 RBAC、ABAC 和 ReBAC。它使组织能够在其应用程序中强制执行动态策略，确保只有授权用户才能访问特定资源。

## 概述

此包提供了两个 LangChain 工具，用于使用 Permit 进行 JWT 验证和权限检查：

* LangchainJWTValidationTool：针对 JWKS 端点验证 JWT 令牌
...
确保您的 PDP（策略决策点）在 PERMIT_PDP_URL 上运行。
...
JWT 验证工具针对 JWKS（JSON Web Key Set）端点验证 JWT 令牌。
...
我们可以通过首先将其绑定到 [支持工具调用的模型](https://docs.permit.io/)，然后调用它来在链中使用我们的工具：

<ChatModelTabs customVarName="llm" />

### 其他演示脚本

对于完全可运行的演示，请查看此 [仓库](https://github.com/permitio/langchain-permit) 中的 `/langchain_permit/examples/demo_scripts` 文件夹。您将找到：
...
只需运行 `python demo_jwt_validation.py` 或 `python demo_permissions_check.py`（在设置好环境变量后）即可看到这些工具的运行效果。

---

## API 参考

有关所有 Permit 功能和配置的详细文档，请访问 API 参考：[docs.permit.io/](https://docs.permit.io/)

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\permit.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
