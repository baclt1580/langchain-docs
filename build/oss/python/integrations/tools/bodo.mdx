...
本笔记本概述了如何使用 [langchain-bodo](https://pypi.org/project/langchain-bodo/) 集成包创建代理并对大型数据集进行问答，该包底层使用了 [Bodo DataFrames](https://github.com/bodo-ai/Bodo) 和 `Python` 代理。

Bodo DataFrames 是一个高性能 DataFrame 库，可以通过简单的导入更改自动加速和扩展 Pandas 代码（见下文示例）。由于其强大的 Pandas 兼容性，Bodo DataFrames 使通常擅长生成 Pandas 代码的大语言模型能够更高效地回答关于更大数据集的问题，并将生成的代码扩展到超出 Pandas 限制的范围。

**注意：`Python` 代理执行 LLM 生成的 Python 代码——如果 LLM 生成的 Python 代码有害，这可能会很糟糕。请谨慎使用。**

## 设置

运行示例之前，复制 [titanic 数据集](https://raw.githubusercontent.com/pandas-dev/pandas/main/doc/data/titanic.csv)
并本地保存为 `titanic.csv`。

安装 langchain-bodo 也将安装依赖项 Bodo 和 Pandas：
...
以下示例借用了 [Pandas DataFrames 代理笔记本](/oss/python/integrations/tools/pandas) 的内容，并做了一些修改以突出关键差异。

第一个示例展示了如何直接将 Bodo DataFrame 传递给 `create_bodo_dataframes_agent` 并提出简单问题。
...
此示例展示了将 Bodo DataFrame 传递给 `create_bodo_dataframes_agent` 并进行一些额外预处理的稍复杂用例。
由于 Bodo DataFrames 是惰性求值的，如果回答问题不需要所有列，则可以节省计算量。请注意，传递给代理的 DataFrame(s) 也可以大于可用内存。
...
您也可以向代理传递多个 DataFrame。
请注意，虽然 Bodo DataFrames 支持 Pandas 中大多数常见的计算密集型操作，但如果代理生成的代码当前不受支持（见下方警告），DataFrames 将被转换回 Pandas 以防止错误。
...
默认情况下，DataFrame(s) 的头部作为 markdown 表嵌入到提示词中。
由于 Bodo DataFrames 是惰性求值的，此 head 操作可以优化，但在某些情况下仍然可能很慢。作为一种优化，您可以将 head 中的行数设置为 0，以便在提示期间不发生评估。
...
您也可以将一个或多个 Pandas DataFrame 传递给 `create_bodo_dataframes_agent`。DataFrame(s) 将在传递给代理之前转换为 Bodo。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\bodo.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
