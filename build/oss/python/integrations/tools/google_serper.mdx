...
本笔记本介绍如何使用 `Google Serper` 组件搜索网络。首先，你需要在 [serper.dev](https://serper.dev) 注册免费账户并获取你的 API 密钥。
...
为了创建使用 Google Serper 工具的代理，请安装 LangGraph

```python
pip install -qU langgraph langchain-openai
```

并使用 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent) 功能初始化一个 ReAct 代理。你还需要设置你的 OPENAI_API_KEY（访问 [platform.openai.com](https://platform.openai.com)）以访问 OpenAI 的聊天模型。
...
如果你还希望以包含元数据的方式结构化地获取结果。为此我们将使用包装器的 `results` 方法。
...
对于所有支持的过滤器，只需前往 [Google Search](https://google.com)，搜索一些内容，点击“工具”，添加日期过滤器，并检查 URL 中的 "tbs="。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\google_serper.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
