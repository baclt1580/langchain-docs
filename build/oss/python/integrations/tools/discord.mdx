...
本指南提供了在 [langchain_discord](/oss/python/integrations/tools/) 中开始使用 Discord 工具的快速概述。有关每个工具和配置的更多详细信息，请查看您仓库中的文档字符串或相关文档页面。
...
以下是完整示例，展示如何在带有 LLM 的链或代理中集成 `DiscordReadMessages` 和 `DiscordSendMessage` 工具。此示例假设您有一个函数（如 [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent)），用于设置能够适当调用工具的 LangChain 风格代理。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\discord.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
