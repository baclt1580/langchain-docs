...
>[Zapier 自然语言操作](https://nla.zapier.com/start/) 通过自然语言 API 界面，让您能够访问 Zapier 平台上的 5000+ 应用和 20000+ 操作。
>
>NLA 支持 `Gmail`、`Salesforce`、`Trello`、`Slack`、`Asana`、`HubSpot`、`Google Sheets`、`Microsoft Teams` 等应用，以及数千种其他应用：[zapier.com/apps](https://zapier.com/apps)
>`Zapier NLA` 处理所有底层 API 认证以及从自然语言 --> 底层 API 调用 --> 返回简化输出给大语言模型的转换。核心思想是，您或您的用户通过类似 OAuth 的设置窗口暴露一组操作，然后您可以通过 REST API 查询并执行这些操作。

NLA 提供 API Key 和 OAuth 两种方式来签署 NLA API 请求。

1. 服务器端（API Key）：适用于快速入门、测试，以及 LangChain 仅使用开发者 Zapier 账户中暴露的操作（并使用开发者在 Zapier.com 上连接的账户）的生产场景。

2. 面向用户（OAuth）：适用于生产场景，您正在部署面向最终用户的应用程序，且 LangChain 需要访问最终用户暴露的操作及其在 Zapier.com 上连接的账户。

本快速入门主要侧重于服务器端用例以节省篇幅。跳转到 [使用 OAuth 访问令牌示例](#example-using-oauth-access-token) 查看简短示例，了解如何为面向用户的场景设置 Zapier。查阅 [完整文档](https://nla.zapier.com/start/) 以获取完整的面向用户 OAuth 开发者支持。

本示例介绍如何使用 Zapier 集成配合 `SimpleSequentialChain`，然后是 `Agent`。
代码如下：
...
以下代码片段展示了如何使用获取到的 OAuth 访问令牌初始化包装器。注意传入的参数与设置环境变量的区别。查阅 [身份验证文档](https://nla.zapier.com/docs/authentication/#oauth-credentials) 以获取完整的面向用户 OAuth 开发者支持。

开发人员负责处理 OAuth 握手以获取和刷新访问令牌。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\zapier.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
