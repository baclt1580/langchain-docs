...
- **核心概念：** 组件概览（数据聚合器、评分器、推理引擎和触发器生成器）。
- **系统架构：** 数据如何在系统中流动以及决策如何制定。
- **自定义：** 如何扩展连接器、评分指标和决策规则以适应您的需求。
...
接下来，我们设置推理引擎和触发器生成器，然后通过实例化 Valthera 工具将所有组件整合在一起。最后，我们执行代理工作流以处理输入消息。
...
此集成当前不支持链式操作。未来的版本可能会包含链式支持。

---

## API 参考

以下是 Valthera 集成提供的主要 API 概览：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\valthera.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
