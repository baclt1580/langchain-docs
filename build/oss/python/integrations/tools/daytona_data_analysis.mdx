...
您必须为 Daytona 配置凭据。您可以通过以下三种方式之一完成此操作：
...
设置 LangSmith 也很有帮助（但不是必需的），以便为您的工具调用提供一流的可见性/<Tooltip tip="记录模型执行的每一步以进行调试和改进">跟踪</Tooltip>。要启用自动跟踪，请设置您的 [LangSmith](/langsmith/home) API 密钥：
...
<Warning>
在完成所有数据分析任务后调用 `tool.close()`，以正确清理资源并避免不必要的费用。
</Warning>
...
有关所有 `DaytonaDataAnalysisTool` 功能和配置的详细文档，请访问 [API 参考](https://www.daytona.io/docs/en/langchain-data-analysis#10-api-reference)。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\daytona_data_analysis.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
