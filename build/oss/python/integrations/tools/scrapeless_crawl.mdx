...
description: "使用 LangChain Python 集成 Scrapeless 爬取工具。"
---

[**Scrapeless**](https://www.scrapeless.com/) 提供灵活且功能丰富的数据采集服务，支持广泛的参数自定义和多格式导出。这些能力使 LangChain 能够更有效地集成和利用外部数据。核心功能模块包括：
...
- **Google 趋势**：从 Google 检索关键词趋势数据，包括随时间变化的热度、地区兴趣和相关搜索。
  - 支持多关键词比较。
  - 支持多种数据类型：`interest_over_time`、`interest_by_region`、`related_queries` 和 `related_topics`。
  - 允许按特定 Google 属性（Web、YouTube、News、Shopping）进行过滤，以进行特定来源的趋势分析。
...
| 类 | 包 | 可序列化 | JS 支持 | 版本 |
| :--- | :--- | :---: | :---: | :---: |
| [`ScrapelessCrawlerScrapeTool`](https://pypi.org/project/langchain-scrapeless/) | [`langchain-scrapeless`](https://pypi.org/project/langchain-scrapeless/) | ✅ | ❌ |  ![PyPI - 版本](https://img.shields.io/pypi/v/langchain-scrapeless?style=flat-square&label=%20) |
| [`ScrapelessCrawlerCrawlTool`](https://pypi.org/project/langchain-scrapeless/) | [`langchain-scrapeless`](https://pypi.org/project/langchain-scrapeless/) | ✅ | ❌ |  ![PyPI - 版本](https://img.shields.io/pypi/v/langchain-scrapeless?style=flat-square&label=%20) |
...
- `urls`（必填，List[str]）：您想要抓取的网站的一个或多个 URL。
- `formats`（可选，List[str]）：定义抓取输出的格式。默认为 `['markdown']`。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主页面内容，不包括标头、导航、页脚等。默认为 True。
...
- `headers`（可选，Dict[str, str]）：请求中发送的自定义标头（例如用于 Cookie 或用户代理）。默认为 None。
- `wait_for`（可选，int）：抓取前等待的毫秒数。有助于给予页面完全加载的时间。默认为 `0`。
- `timeout`（可选，int）：请求超时（毫秒）。默认为 `30000`。

### ScrapelessCrawlerCrawlTool

ScrapelessCrawlerCrawlTool 允许您使用 Scrapeless 的 Crawler Crawl API 从基础 URL 开始爬取网站。它支持 URL 的高级过滤、爬取深度控制、内容抓取选项、标头自定义等。

该工具接受以下参数：

- `url`（必填，str）：开始爬取的基础 URL。

- `limit`（可选，int）：要爬取的最大页数。默认为 `10000`。
- `include_paths`（可选，List[str]）：要包含在爬取中的 URL 路径名正则表达式模式。仅包含匹配这些模式的 URL。例如，设置 `["blog/.*"]` 将仅包含 `/blog/` 路径下的 URL。默认为 None。
- `exclude_paths`（可选，List[str]）：要从爬取中排除的 URL 路径名正则表达式模式。例如，设置 `["blog/.*"]` 将排除 `/blog/` 路径下的 URL。默认为 None。
- `max_depth`（可选，int）：相对于基础 URL 的最大爬取深度，通过 URL 路径中的斜杠数量测量。默认为 `10`。
- `max_discovery_depth`（可选，int）：基于发现顺序的最大爬取深度。根和站点地图页面的深度为 `0`。例如，设置为 `1` 并忽略站点地图将仅爬取输入的 URL 及其直接链接。默认为 None。
- `ignore_sitemap`（可选，bool）：爬取期间是否忽略网站站点地图。默认为 False。
- `ignore_query_params`（可选，bool）：是否忽略查询参数差异以避免重新抓取相似 URL。默认为 False。
- `deduplicate_similar_urls`（可选，bool）：是否去重相似 URL。默认为 True。
- `regex_on_full_url`（可选，bool）：正则匹配是否应用于完整 URL 而不仅仅是路径。默认为 True。
- `allow_backward_links`（可选，bool）：是否允许爬取 URL 层级之外的反向链接。默认为 False。
- `allow_external_links`（可选，bool）：是否允许爬取指向外部网站的链接。默认为 False。
- `delay`（可选，int）：页面抓取之间的秒数延迟，以遵守速率限制。默认为 `1`。
- `formats`（可选，List[str]）：抓取内容的格式。默认为 ["markdown"]。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content`（可选，bool）：是否仅返回主要内容，不包括标头、导航栏、页脚等。默认为 True。
...
- `headers`（可选，Dict[str, str]）：请求中发送的自定义 HTTP 标头，例如 Cookie 或用户代理字符串。默认为 None。
- `wait_for`（可选，int）：抓取内容前等待的毫秒数，允许页面完全加载。默认为 `0`。
- `timeout`（可选，int）：请求超时（毫秒）。默认为 `30000`。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\scrapeless_crawl.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
