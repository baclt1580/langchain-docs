...
>[Golden](https://golden.com) 提供了一套自然语言 API，用于使用 Golden 知识图谱进行查询和丰富，例如：诸如 `Products from OpenAI`、`Generative ai companies with series a funding` 以及 `rappers who invest` 等查询可用于检索相关实体的结构化数据。
>
>The `golden-query` LangChain 工具是对 [Golden Query API](https://docs.golden.com/reference/query-api) 的封装，可实现对这些结果的编程访问。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\golden_query.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
