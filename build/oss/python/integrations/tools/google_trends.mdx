---
title: "Google 趋势集成"
description: "使用 LangChain Python 与 Google 趋势工具进行集成。"
---

本笔记本介绍如何使用 Google Trends 工具获取趋势信息。

首先，你需要在 [serpapi.com/users/sign_up](https://serpapi.com/users/sign_up) 注册一个 `SerpApi key`。

然后，你必须使用以下命令安装 `google-search-results`：

`pip install google-search-results`

然后，你需要将环境变量 `SERPAPI_API_KEY` 设置为你自己的 `SerpApi key`

[或者，你可以将密钥作为参数传递给包装器 `serp_api_key="your secret key"`]
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\google_trends.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
