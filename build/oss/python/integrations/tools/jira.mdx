...
- API token 认证：设置 JIRA_API_TOKEN（如果需要则包括 JIRA_USERNAME）环境变量
- OAuth2.0 认证：将 JIRA_OAUTH2 环境变量设置为一个字典，其字段为 "client_id" 和 "token"，其中 "token" 是一个至少包含 "access_token" 和 "token_type" 的字典
