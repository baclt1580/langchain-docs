...
[CAMB AI](https://camb.ai) 提供多语言音频和本地化服务，支持 140+ 种语言，包括文本转语音、翻译、转录、语音克隆和音频生成。
...
|:------|:--------|:------------:|:--------------------------------------------------------------:|:--------:|
| [`CambToolkit`](https://github.com/camb-ai/langchain-camb) | [`langchain-camb`](https://pypi.org/project/langchain-camb/) | beta | ❌ | ![PyPI - 版本](https://img.shields.io/pypi/v/langchain-camb?style=flat-square&label=%20) |
...
| `CambTranslationTool` | 140+ 种语言之间的文本翻译 | 翻译后的文本 |
| `CambTranscriptionTool` | 带说话人识别的语音转文本 | 包含文本和片段的 JSON |
| `CambVoiceListTool` | 列出 TTS 可用语音 | 语音列表 JSON |
| `CambVoiceCloneTool` | 从 2 秒以上的音频样本克隆语音 | 新语音 ID |
| `CambTextToSoundTool` | 根据文生成本音乐和音效 | 音频文件路径 |
| `CambAudioSeparationTool` | 从背景音频中分离人声 | 包含音频路径的 JSON |

## 设置

要访问 CAMB AI 工具，您需要创建一个 CAMB AI 账户并从 [camb.ai](https://camb.ai) 获取 API 密钥。
...
设置 LangSmith 以获得最佳的可观测性/<Tooltip tip="记录模型执行的每一步以调试和改进它">追踪</Tooltip>也很有帮助（但不是必需的）。要启用自动追踪，请设置您的 [LangSmith](/langsmith/home) API 密钥：
...
您可以使用 `CambToolkit` 一次性获取所有工具，或者单独实例化工具。
...
转录音频，将其翻译，并用另一种语言生成语音——这是配音工作流的基础：
...
您可以将 CAMB AI 工具与 LangGraph 代理一起使用，以创建强大的多语言 AI 助手：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\camb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
