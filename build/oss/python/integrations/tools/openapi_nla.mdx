...
description: "使用 LangChain Python 与 Natural language API toolkits 工具进行集成。"
---

`Natural Language API` 工具包（`NLAToolkits`）允许 LangChain 代理高效地规划和组合跨端点的调用。
...
某些端点可能需要通过访问令牌等进行用户身份验证。此处我们展示如何通过 `Requests` 包装器对象传入认证信息。

由于每个 NLATool 都为其包装的 API 提供了一个简洁的自然语言接口，顶级对话代理在整合各个端点以满足用户请求方面的工作更容易。

**添加 Spoonacular 端点。**

1. 前往 [Spoonacular API 控制台](https://spoonacular.com/food-api/console#Profile) 并注册免费账户。
2. 点击 `Profile` 并复制您的 API 密钥如下。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\openapi_nla.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
