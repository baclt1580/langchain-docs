---
title: "Google Books 集成"
description: "使用 LangChain Python 与 Google Books 工具进行集成。"
---

支持 ReAct 模式并允许您搜索 Google Books API 的 Google Books 工具。Google Books 是世界上最大的 API，以策划的方式跟踪书籍。它拥有超过 4000 万条条目，可以为用户提供大量数据。
...
您需要一个来自 Google Books 的 API 密钥。您可以通过访问并按照以下步骤操作：[https://developers.google.com/books/docs/v1/using#APIKey](https://developers.google.com/books/docs/v1/using#APIKey)。

然后，您需要将环境变量 `GOOGLE_BOOKS_API_KEY` 设置为您的 Google Books API 密钥。

## 实例化

要实例化工具，请导入 Google Books 工具并设置您的凭据。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\tools\google_books.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
