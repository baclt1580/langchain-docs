...
description: "使用 LangChain Python 集成拆分 JSON 数据的文本分割器。"
---

此 JSON 分割器 [拆分](/oss/python/integrations/splitters/) JSON 数据，同时允许控制块大小。它深度优先遍历 JSON 数据并构建较小的 JSON 块。它尝试保持嵌套的 JSON 对象完整，但如果需要，为了将块保持在 min_chunk_size 和 max_chunk_size 之间，也会拆分它们。

如果值不是嵌套 JSON，而是一个非常大的字符串，则该字符串不会被拆分。如果您需要对块大小进行硬性限制，请考虑将这些块与递归文本分割器组合使用。有一个可选的预处理步骤来拆分列表，首先将它们转换为 JSON（字典），然后按此方式拆分。
...
```
要获取 LangChain [文档](https://reference.langchain.com/python/langchain-core/documents/base/Document) 对象，请使用 `.create_documents` 方法：
...
指定 `convert_lists=True` 以预处理 JSON，将列表内容转换为字典，其中 `index:item` 作为 `key:val` 对：
...
```
列表已转换为字典，但即使拆分为多个块，也保留了所有必要的上下文信息：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\splitters\recursive_json_splitter.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
