...
在 MacOS 上，iMessage 将对话存储在一个位于 `~/Library/Messages/chat.db` 的 sqlite 数据库中（至少对于 macOS Ventura 13.4 如此）。
`IMessageChatLoader` 从这个数据库文件加载数据。

1. 创建 `IMessageChatLoader`，并将文件路径指向你想要处理的 `chat.db` 数据库。
...
你的终端很可能被拒绝访问 `~/Library/Messages`。要使用这个类，你可以将数据库复制到一个可访问的目录（例如，文稿）并从那里加载。或者（不推荐），你可以在系统设置 > 安全性与隐私 > 完全磁盘访问中为你的终端模拟器授予完全磁盘访问权限。

我们创建了一个示例数据库，你可以通过[此链接的云端文件](https://drive.google.com/file/d/1NebNKqTA2NXApCmeH6mu0unJD2tANZzo/view?usp=sharing)使用。
...
向加载器提供 zip 目录的文件路径。你可以选择性地指定映射到 AI 消息的用户 ID，以及配置是否合并消息连续块。
...
`load()`（或 `lazy_load`）方法返回一个 "ChatSessions" 列表，目前只包含每个加载对话的消息列表。所有消息最初都被映射为 "HumanMessage" 对象。

你可以选择性地合并消息"连续块"（同一发送者的连续消息），并选择一个发送者代表 "AI"。经过微调的 LLM 将学会生成这些 AI 消息。
...
# 将来自 "Tortoise" 的消息转换为 AI 消息。你能猜出这些对话是谁之间的吗？
...
是时候微调模型了。确保已安装 `openai` 并正确设置了你的 `OPENAI_API_KEY`
...
你可以直接将生成的模型 ID 用于 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) 模型类。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\chat_loaders\imessage.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
