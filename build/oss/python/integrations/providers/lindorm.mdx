---
title: "Lindorm 集成"
description: "使用 LangChain Python 与 Lindorm 集成。"
---

Lindorm 是阿里云原生多模态数据库，支持统一访问和集成处理各种类型的数据，包括宽表、时序、文本、对象、流和空间数据。它兼容多种标准接口，如 SQL、HBase/Cassandra/S3、TSDB、HDFS、Solr 和 Kafka，并能与第三方生态工具无缝集成。这使其适用于日志、监控、计费、广告、社交网络、旅行和风控等场景。Lindorm 也是支持阿里巴巴核心业务的数据库之一。

要使用 Lindorm 的 AI 和向量功能，您应该 [获取服务](https://help.aliyun.com/document_detail/174640.html?spm=a2c4g.11186623.help-menu-172543.d_0_1_0.4c6367558DN8Uq) 并安装 `langchain-lindorm-integration` 包。

```python
!pip install -U langchain-lindorm-integration
```

## 嵌入

要使用 Lindorm AI 服务中部署的嵌入模型，请导入 LindormAIEmbeddings。

```python
from langchain_lindorm_integration import LindormAIEmbeddings
```

## 重排序

Lindorm AI 服务也支持重排序。

```python
from langchain_lindorm_integration.reranker import LindormAIRerank
```

## 向量存储

Lindorm 还支持向量存储。

```python
from langchain_lindorm_integration import LindormVectorStore
```

## 字节存储

使用 Lindorm 的 ByteStore

```python
from langchain_lindorm_integration import LindormByteStore
```

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\lindorm.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
<Callout icon="terminal-2">
    [Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.
</Callout>
</div>
