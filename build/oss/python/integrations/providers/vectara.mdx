...
[Vectara](https://vectara.com/) 是值得信赖的 AI 助手和代理平台，专注于关键任务应用程序的企业就绪性。
Vectara 无服务器 RAG-as-a-service 通过易于使用的 API 提供 RAG 的所有组件，包括：
...
- [API 沙盒](https://docs.vectara.com/docs/rest-api/)
- [快速入门](https://docs.vectara.com/docs/quickstart)

本笔记本展示了如何使用基本检索功能，当仅将 Vectara 用作向量存储（无摘要）时，包括：`similarity_search` 和 `similarity_search_with_score` 以及使用 LangChain 的 `as_retriever` 功能。
...
2. 在您的账户中，您可以创建一个或多个语料库。每个语料库代表一个区域，用于在从输入文档导入时存储文本数据。要创建语料库，请使用 **"创建语料库"** 按钮。然后为您的语料库提供名称和描述。可选地，您可以定义过滤属性并应用一些高级选项。如果您点击创建的语料库，您可以在顶部看到其名称和语料库 ID。
3. 接下来，您需要创建 API 密钥以访问语料库。点击语料库视图中的 **"访问控制"** 标签页，然后点击 **"创建 API 密钥"** 按钮。为您的密钥命名，并选择您的密钥是仅查询还是查询 + 索引。点击“创建”，您现在拥有一个活动的 API 密钥。请保密此密钥。

要将 LangChain 与 Vectara 配合使用，您需要拥有这两个值：`corpus_key` 和 `api_key`。
您可以通过两种方式向 LangChain 提供 `VECTARA_API_KEY`：

1. 在您的环境中包含这些变量：`VECTARA_API_KEY`。
...
请注意，我们使用了 add_files 接口，它不需要任何本地处理或分块——Vectara 接收文件内容，并对文件执行所有必要的预处理、分块和嵌入到其知识库中。
...
- 我们启用摘要，指定我们希望 LLM 选择前 7 个匹配块并用英语响应
...
有关 Vectara 作为向量存储的更多详细信息，[请转到此笔记本](../vectorstores/vectara.ipynb)。

## Vectara 聊天

在使用 LangChain 创建聊天机器人的大多数情况下，必须集成为一个特殊的 `memory` 组件，该组件维护聊天会话的历史记录，然后使用该历史记录确保聊天机器人了解对话历史。

使用 Vectara Chat——所有这些均由 Vectara 在后台自动完成。
...
Vectara 提供智能查询重写选项，该选项通过从自然语言查询自动生成元数据过滤器表达式来提高搜索精度。此功能分析用户查询，提取相关元数据过滤器，并重新表述查询以关注核心信息需求。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\vectara.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
