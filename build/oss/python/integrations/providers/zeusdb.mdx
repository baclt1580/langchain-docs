...
如果您愿意，也可以使用此包与其他任何嵌入提供商（Hugging Face、Cohere、自定义函数等）配合使用。
...
ZeusDB 的企业级功能完全集成到 LangChain 生态系统中，提供量化、持久化、高级搜索功能以及许多其他企业级功能。
...
ZeusDB 持久化允许您将完全填充的索引保存到磁盘，并在以后加载它，同时完整恢复状态。这包括向量、元数据、HNSW 图，以及（如果启用）产品量化模型。
...
- 路径是目录，而不是单个文件。确保目标可写。
- 保存的索引跨平台，并包含格式/版本信息以进行兼容性检查。
- 如果您使用了 PQ，则压缩模型和状态都会保留——加载后无需重新训练。
- 您可以在 loaded_store 上继续使用所有向量存储 API（similarity_search、retrievers 等）。
...
使用这些来控制搜索的评分、多样性、元数据过滤和检索器集成。

#### 带分数的相似度搜索

返回来自 ZeusDB 的 `(Document, raw_distance)` 对（距离越低越相似）。
...
MMR（最大边际相关性）平衡两种力量：与查询的相关性和选定结果之间的多样性，减少近重复答案。使用 lambda_mult 控制权衡（1.0 = 全部相关性，0.0 = 全部多样性）。
...
将向量存储转换为检索器可为您提供标准的 LangChain 接口，链（例如 RetrievalQA）可以调用它以获取上下文。底层使用您选择的搜索类型（similarity 或 mmr）和 search_kwargs。
...
ZeusDB 支持异步操作，以实现非阻塞、并发的向量操作。

**何时使用异步：** Web 服务器（FastAPI/Starlette）、执行并行搜索的智能体/管道，或希望非阻塞/并发检索的笔记本。如果您编写简单脚本，同步方法即可。

这些是**异步操作** - 常规同步方法的 async/await 版本。以下是每个操作的作用：

1. `await vector_store.aadd_documents(documents)` - 异步向向量存储添加文档（`add_documents()` 的异步版本）
2. `await vector_store.asimilarity_search("query", k=5)` - 异步执行相似度搜索（`similarity_search()` 的异步版本）
...
ZeusDB 包含企业级结构化日志记录，可与智能环境检测自动配合工作：
...
要了解 ZeusDB 企业级日志功能的完整功能，请阅读以下 [文档](https://docs.zeusdb.com/en/latest/vector_database/logging.html)。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\zeusdb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
