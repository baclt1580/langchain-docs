...
LangFair 是一个全面的 Python 库，旨在对大型语言模型（LLM）用例进行偏见和公平性评估。LangFair [仓库](https://github.com/cvs-health/langfair) 包含一个全面的框架，用于 [选择偏见和公平性指标](https://github.com/cvs-health/langfair/tree/main#-choosing-bias-and-fairness-metrics-for-an-llm-use-case)，以及 [演示笔记本](https://github.com/cvs-health/langfair/tree/main/examples) 和一份 [技术手册](https://arxiv.org/abs/2407.10853)，其中讨论了 LLM 的偏见和公平性风险、评估指标和最佳实践。

访问我们的 [文档站点](https://cvs-health.github.io/langfair/) 以获取使用 LangFair 的详细说明。
...
### 使用示例
以下是代码示例，说明了如何使用 LangFair 评估文本生成和摘要用例中的偏见和公平性风险。以下示例假设用户已经定义了来自其用例的提示列表 `prompts`。

##### 生成 LLM 响应
要生成响应，我们可以使用 LangFair 的 `ResponseGenerator` 类。首先，我们必须创建一个 `langchain` LLM 对象。下面我们使用 `ChatVertexAI`，但也可以使用 **[LangChain 的任何 LLM 类](https://js.langchain.com/docs/integrations/chat/)**。注意，`InMemoryRateLimiter` 用于避免速率限制错误。
```python
...
##### 计算毒性指标
可以使用 `ToxicityMetrics` 计算毒性指标。注意，使用 `torch.device` 是可选的，如果可用 GPU 则应使用它来加快毒性计算速度。
```python
...
为了简化文本生成和摘要用例的评估，`AutoEval` 类执行多步过程，仅用两行代码即可完成上述所有步骤。
```python
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\langfair.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
