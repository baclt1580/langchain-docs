...
了解如何在 Predibase 上使用 LangChain 与模型配合。
...
Predibase 通过实现 LLM 模块与 LangChain 集成。您可以在下方看到简短示例，或在 LLM > 集成 > Predibase 下查看完整笔记本。
...
Predibase 还支持基于 `model` 参数给定的基础模型进行微调的 Predibase 托管和 HuggingFace 托管适配器：
...
Predibase 还支持基于 `model` 参数给定的基础模型进行微调的适配器：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\predibase.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
