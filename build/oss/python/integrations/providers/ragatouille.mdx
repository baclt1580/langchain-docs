...
>[RAGatouille](https://github.com/bclavie/RAGatouille) 让使用 `ColBERT` 变得极其简单！ [ColBERT](https://github.com/stanford-futuredata/ColBERT) 是一种快速且准确的检索模型，能够在数十毫秒内实现对大型文本集合的可扩展基于 BERT 的搜索。
>
>请参阅 [ColBERTv2: Effective and Efficient Retrieval via Lightweight Late Interaction](https://arxiv.org/abs/2112.01488) 论文。

我们可以以多种方式使用 RAGatouille。
...
我们可以将 RAGatouille 用作检索器。有关此更多信息，请参阅 [RAGatouille 检索器](/oss/python/integrations/retrievers/ragatouille)

## 文档压缩器

我们还可以直接使用 RAGatouille 作为重排序器。这将允许我们使用 ColBERT 对任何通用检索器检索到的结果进行重排序。这样做的好处是，我们可以在任何现有索引之上执行此操作，因此不需要创建新索引。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\ragatouille.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
