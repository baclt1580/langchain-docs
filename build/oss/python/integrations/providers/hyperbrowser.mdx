...
您可以在 [Hyperbrowser 浏览器代理工具指南](/oss/python/integrations/tools/hyperbrowser_browser_agent_tools) 中查看更多详细信息。

#### Browser Use 工具
一种通用的浏览器自动化工具，可以通过自然语言指令处理各种网页任务。
...
#### OpenAI CUA 工具
利用 OpenAI 的计算机使用代理能力进行高级网页交互和信息收集。
...
以下是 Hyperbrowser 提供的网页抓取工具的简要描述。您可以在 [Hyperbrowser 网页抓取工具指南](/oss/python/integrations/tools/hyperbrowser_web_scraping_tools) 中查看更多详细信息。
...
#### 爬取工具
Crawl 工具使您能够遍历整个网站，从给定的 URL 开始，并配置页面限制。
...
`langchain-hyperbrowser` 中的 `HyperbrowserLoader` 类可以轻松用于从任何单页或多页加载内容，以及爬取整个站点。
...
您可以指定加载器执行的操作。默认操作是 `scrape`。对于 `scrape`，您可以提供单个 URL 或要抓取的 URL 列表。对于 `crawl`，您只能提供单个 URL。`crawl` 操作将爬取提供的页面和子页面，并为每个页面返回一个文档。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\hyperbrowser.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
