...
Log10 是一个 [开源](https://github.com/log10-io/log10) 的无代理 LLM 数据管理和应用开发平台，让您能够记录、调试和标记您的 LangChain 调用。

## 快速开始

1. 在 [log10.io](https://log10.io) 创建您的免费账户
2. 分别将设置和组织选项卡中的 `LOG10_TOKEN` 和 `LOG10_ORG_ID` 作为环境变量添加。
3. 此外，将 `LOG10_URL=https://log10.io` 以及您常用的 LLM API 密钥（例如 `OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`）添加到您的环境中
...
您还可以混合使用直接 OpenAI 调用和 LangChain LLM 调用：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\providers\log10.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
