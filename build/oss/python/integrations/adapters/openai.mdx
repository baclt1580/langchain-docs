---
title: "OpenAI 适配器集成"
description: "使用 LangChain Python 集成 OpenAI 适配器。"
---

**请确保 OpenAI 库版本为 1.0.0 或更高；否则，请参考旧版文档 [OpenAI 适配器（旧版）](/oss/python/integrations/adapters/openai-old/)。**

许多人从 OpenAI 开始，但希望探索其他模型。LangChain 与众多模型提供商的集成使得这一点变得容易实现。虽然 LangChain 拥有自己的消息和模型 API，但我们也通过公开一个适配器，将 LangChain 模型适配到 OpenAI API，尽可能简化了探索其他模型的过程。

目前，这仅处理输出，不返回其他信息（如 token 计数、停止原因等）。
...
```python
lc_result["choices"][0]["message"]  # 也支持索引访问
```
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\python\integrations\adapters\openai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
