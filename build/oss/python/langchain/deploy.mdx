...
你的应用代码必须存放在 GitHub 仓库中，才能在 LangSmith 上部署。支持公共和私有仓库。在本快速入门中，首先请按照[本地服务器设置指南](/oss/python/langchain/studio#set-up-local-agent-server)确保你的应用兼容 LangGraph。然后，将你的代码推送到仓库。
