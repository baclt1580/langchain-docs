---
title: AI Elements
description: 基于 shadcn/ui 的可组合组件库，专为 AI 聊天界面设计，可与 useStream 无缝集成
---

[AI Elements](https://elements.ai-sdk.dev/) 是一个基于 shadcn/ui 的可组合组件库，专为 AI 聊天界面构建。诸如 `Conversation`、`Message`、`Tool`、`Reasoning` 和 `PromptInput` 等组件设计为可直接放入任何 React 项目，并通过最少的胶水代码连接到 `stream.messages`。
...
<Tip>
  克隆并运行 [完整的 AI Elements 示例](https://github.com/langchain-ai/langgraphjs/tree/main/examples/ai-elements)，以在实际项目中查看工具调用渲染、推理显示、消息流式传输等功能。
</Tip>
...
*   **使用 `isInstance` 进行类型守卫：** 使用 `HumanMessage.isInstance(msg)` 和 `AIMessage.isInstance(msg)` 而不是检查 `msg.getType()`，以获得正确的 TypeScript 类型收窄

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\langchain\frontend\integrations\ai-elements.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
