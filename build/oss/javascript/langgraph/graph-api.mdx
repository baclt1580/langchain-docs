...
编译是一个相当简单的步骤。它对图的结构提供一些基本检查（例如没有孤立的节点等）。这也是您可以指定运行时参数的地方，如 [checkpointers](/oss/javascript/langgraph/persistence) 和 breakpoints。您只需调用 `.compile` 方法即可编译您的图：
...
还可以为图定义显式的输入和输出模式。在这些情况下，我们定义一个包含与图操作相关的 _所有_ 键的“内部”模式。但是，我们也定义 `input` 和 `output` 模式，它们是“内部”模式的子集，以约束图的输入和输出。有关更多详细信息，请参阅 [定义输入和输出模式](/oss/javascript/langgraph/use-graph-api#define-input-and-output-schemas)。
...
`messages` 字段定义为 `MessagesValue`，它是带有内置归约器的 [`BaseMessage`](https://reference.langchain.com/javascript/langchain-core/messages/BaseMessage) 对象列表。通常，需要跟踪的状态不仅仅是消息，所以我们看到人们扩展此状态并添加更多字段，例如：
...
1. `state`—图的 [状态](#state)
2. `config`—包含配置信息（如 `thread_id`）和追踪信息（如 `tags`）的 [`RunnableConfig`](https://reference.langchain.com/javascript/langchain-core/runnables/RunnableConfig) 对象
...
在幕后，函数被转换为 [`RunnableLambda`](https://reference.langchain.com/javascript/langchain-core/runnables/RunnableLambda)，为您的函数添加批量和异步支持，以及 [原生追踪和调试](/langsmith/home)。
...
[`START`](https://reference.langchain.com/javascript/langchain-langgraph/index/START) 节点是一个特殊节点，代表向图发送用户输入的节点。引用此节点的主要目的是确定哪些节点应该首先被调用。
...
如果您想**可选地**路由到一个或多个边（或可选地终止），可以使用 [`addConditionalEdges`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addConditionalEdges) 方法。此方法接受一个节点名称和一个在该节点执行后要调用的“路由函数”：
...
入口点是图启动时运行的第一个节点。您可以使用来自虚拟 [`START`](https://reference.langchain.com/javascript/langchain-langgraph/index/START) 节点到要执行的第一节点的 [`addEdge`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addEdge) 方法来指定进入图的位置。
...
条件入口点允许您根据自定义逻辑从不同的节点开始。您可以使用来自虚拟 [`START`](https://reference.langchain.com/javascript/langchain-langgraph/index/START) 节点的 [`addConditionalEdges`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addConditionalEdges) 来完成此操作。
...
- `resume`：提供值以在 [中断](/oss/javascript/langgraph/interrupts) 后恢复执行。
...
当您既需要更新状态**又**需要路由到不同节点时使用 [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command)。如果您只需要路由而不更新状态，请使用 [条件边](#conditional-edges)。

在节点函数中使用 [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) 时，您必须在添加节点时添加 `ends` 参数以指定它可以路由到的节点：
...
[`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) 仅添加动态边—使用 `add_edge` / `addEdge` 定义的静态边仍然执行。例如，如果 `node_a` 返回 `Command(goto="my_other_node")`，并且您也有 `graph.add_edge("node_a", "node_b")`，则 `node_b` 和 `my_other_node` 都将运行。
...
如果您正在使用 [子图](/oss/javascript/langgraph/use-subgraphs)，可以通过在 `Command` 中指定 `graph: Command.PARENT` 从子图中的节点导航到父图中的不同节点：
...
使用 `new Command({ resume: ... })` 提供值并在 [中断](/oss/javascript/langgraph/interrupts) 后恢复图执行。传递给 `resume` 的值成为暂停节点内 `interrupt()` 调用的返回值：
...
有关详细信息，请参阅 [在工具中使用](/oss/javascript/langgraph/use-graph-api#use-inside-tools)。
...
有关配置的完整分解，请参阅 [添加运行时配置](/oss/javascript/langgraph/use-graph-api#add-runtime-configuration)。
...
能够可视化图通常很好，尤其是随着它们变得越来越复杂。LangGraph 自带几种内置方式来可视化图。有关更多信息，请参阅 [可视化您的图](/oss/javascript/langgraph/use-graph-api#visualize-your-graph)。
