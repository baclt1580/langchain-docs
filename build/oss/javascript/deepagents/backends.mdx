...
沙盒和 [`LocalShellBackend`](https://reference.langchain.com/javascript/deepagents/backends/LocalShellBackend) 还提供了一个 `execute` 工具。
...
**工作原理：**
- 通过 [`StateBackend`](https://reference.langchain.com/javascript/deepagents/backends/StateBackend) 将文件存储在 LangGraph 智能体状态中，用于当前线程。
...
3. 对于需要文件系统交互的生产环境，使用[沙盒后端](/oss/javascript/deepagents/sandboxes)。
...
3. 对于需要 shell 执行的生产环境，使用[沙盒后端](/oss/javascript/deepagents/sandboxes)。
...
<Tip>
    `namespace` 参数控制数据隔离。对于多用户部署，始终设置一个[命名空间工厂](/oss/javascript/deepagents/backends#namespace-factories)以按用户或租户隔离数据。
</Tip>
...
<Note>
    当未提供命名空间工厂时，遗留的默认值使用 LangGraph 配置元数据中的 `assistant_id`。这意味着同一[助手](/langsmith/assistants)的所有用户共享相同的存储。对于多用户[投入生产](/oss/javascript/deepagents/going-to-production)，请始终提供命名空间工厂。
</Note>
