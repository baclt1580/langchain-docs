...
[SAP HANA Cloud 向量引擎](https://www.sap.com/events/teched/news-guide/ai.html#article8) 是一个完全集成在 `SAP HANA Cloud 数据库` 中的向量存储。
...
// 此函数将创建一个表 "test_fromTexts"（如果不存在），如果存在，
// 则值将被追加到表中。
...
console.log(`过滤条件：${JSON.stringify(advancedFilter)}`);
printFilterResult(
...
/* 过滤条件：{"$or":[{"id":1},{"id":2},{"id":3}]}
{ name: 'adam', is_active: true, id: 1, height: 10 }
{ name: 'bob', is_active: false, id: 2, height: 5.7 }
{ name: 'jane', is_active: true, id: 3, height: 2.4 } */

// 你也可以使用 $and 和 $or 定义嵌套过滤条件。
advancedFilter = {
...
/* 过滤条件：{"$and":[{"$or":[{"id":1},{"id":2}]},{"height":{"$gte":5.0}}]}
{ name: 'adam', is_active: true, id: 1, height: 10 }
{ name: 'bob', is_active: false, id: 2, height: 5.7 } */
...
## 在链中使用 VectorStore 作为检索器以实现检索增强生成 (RAG)
...
const vectorStore = new HanaDB(embeddings, args);
await vectorStore.initialize();
// 将存储用作链的一部分，前提是 "test_fromDocs" 存在并包含分块文档。
const model = new ChatOpenAI({ model: "gpt-3.5-turbo-1106" });
const questionAnsweringPrompt = ChatPromptTemplate.fromMessages([
  [
    "system",
    "你是国情咨文主题的专家。你获得了多个与需要回答的提示相关的上下文项。使用以下上下文片段来回答最后的问题。\n\n{context}",
  ],
  ["human", "{input}"],
]);
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\hanavector.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
