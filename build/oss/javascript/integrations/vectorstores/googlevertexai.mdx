...
description: "使用 LangChain JavaScript 与 Google Vertex AI 匹配引擎向量存储进行集成。"
---
...
Google Vertex AI 匹配引擎“提供业界领先的高规模、低延迟向量数据库。这些向量数据库通常被称为向量相似性匹配或近似最近邻（ANN）服务。”

## 设置

<Warning>
**本模块要求端点及已部署的索引已预先创建**，因为创建过程耗时接近一小时。了解更多信息，请参阅 LangChain Python 文档中的[创建索引并将其部署到端点](https://python.langchain.com/docs/integrations/vectorstores/google_vertex_ai_vector_search/#deploy-index-to-the-endpoint)。
</Warning>

在运行此代码之前，请确保已在 Google Cloud 控制台为相关项目启用了 Vertex AI API，并且已通过以下任一方式完成 Google Cloud 身份验证：

- 您已登录具有项目权限的账户（使用 `gcloud auth application-default login`）。
- 您正在使用具有项目权限的服务账户的机器上运行。
- 您已下载具有项目权限的服务账户的凭证文件，并将 `GOOGLE_APPLICATION_CREDENTIALS` 环境变量设置为该文件的路径。
...
```
匹配引擎不存储实际文档内容，仅存储嵌入向量。因此，您需要一个文档存储。以下示例使用 Google Cloud Storage，这需要安装：
...
创建 `MatchingEngine` 对象时，您需要一些关于匹配引擎配置的信息。您可以从 Matching Engine 的 Cloud Console 获取这些信息：

- 索引的 ID
- 索引端点的 ID

您还需要一个文档存储。虽然 `InMemoryDocstore` 适合初步测试，但您可能需要使用类似 [GoogleCloudStorageDocstore](https://reference.langchain.com/javascript/langchain-community/stores/doc/gcs/GoogleCloudStorageDocstore) 的存储来更持久地保存文档。
...
```
文档中的任何元数据都会被转换为匹配引擎的“允许列表”值，可在查询时用于过滤。
...
```
文档应具有可用的“id”参数。如果未设置，系统将分配一个 ID 并作为 Document 的一部分返回。

### 查询文档

执行标准的 k-最近邻搜索并返回所有结果，可使用以下任一标准方法：
...
我们可以根据为文档设置的元数据来限制返回的文档。如果只想限制结果为红色，可以这样做：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\googlevertexai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
