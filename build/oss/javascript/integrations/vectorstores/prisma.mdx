...
为了在 PostgreSQL 数据库中增强现有模型的向量搜索能力，LangChain 支持将 [Prisma](https://www.prisma.io/) 与 PostgreSQL 及 [`pgvector`](https://github.com/pgvector/pgvector) Postgres 扩展结合使用。

## 设置

### 使用 Supabase 设置数据库实例

参考 [Prisma 与 Supabase 集成指南](https://supabase.com/docs/guides/integrations/prisma) 来使用 Supabase 和 Prisma 设置一个新的数据库实例。
...
假设您尚未创建模型，请创建一个带有 `vector` 字段的新模型，其类型为 `Unsupported("vector")`：
...
如果尚未启用 `pgvector` 扩展，请在新建的迁移中添加以下行以启用它：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\prisma.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
