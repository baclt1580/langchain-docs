...
- **查询和索引服务**：使用 Couchbase 的查询服务及 SQL++，而非搜索服务
...
### 超大规模索引
一种专门的向量索引，利用 Couchbase 的向量索引能力优化向量操作：
...
- `IndexType.HYPERSCALE` - 专门的向量索引，用于获得最佳向量搜索性能
...
2. **选择索引类型**：
   - 使用**超大规模索引**处理纯向量搜索工作负载，主要执行相似性搜索
   - 使用**复合索引**处理混合查询，将向量相似性与标量字段过滤结合
3. **调优参数**：根据数据大小和性能要求调整 `indexScanNprobes` 和 `indexTrainlist`
4. **尽早过滤**：使用 WHERE 子句在向量计算前减少搜索空间
...
不需要，与基于搜索的 `CouchbaseSearchVectorStore` 不同，基于查询的实现可以在没有预先创建索引的情况下工作。但是，创建适当的向量索引（超大规模或复合）将显著提高查询性能。

### 何时应该使用超大规模索引与复合索引？

- 当您主要执行向量相似性搜索，对其他字段过滤最少时，使用**超大规模索引**
- 当您经常在同一查询中将向量相似性与标量字段过滤结合时，使用**复合索引**
...
可以，两者都可以在同一文档结构上工作。但是，它们使用不同的服务（搜索 vs 查询）并且有不同的索引要求。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\couchbase_query.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
