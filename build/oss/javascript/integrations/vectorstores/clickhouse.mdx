...
[ClickHouse](https://clickhouse.com/) 是一个强大且开源的列式数据库，专为处理分析查询和高效存储而设计，旨在提供向量搜索与分析功能的强大组合。

## 设置步骤

1. 启动一个 ClickHouse 集群。详情请参考 [ClickHouse 安装指南](https://clickhouse.com/docs/en/getting-started/install/)。
2. 启动 ClickHouse 集群后，从集群的 `操作` 菜单中获取 `连接详情`。您将需要主机地址、端口、用户名和密码。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\clickhouse.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
