...
description: "使用 LangChain JavaScript 与 Couchbase 搜索向量存储集成。"
---

`CouchbaseSearchVectorStore` 是向量搜索的一种实现，属于 Couchbase 中[全文搜索服务](https://docs.couchbase.com/server/current/learn/services-and-indexes/services/search-service.html)（搜索服务）的一部分。

本教程解释了如何通过 Couchbase 搜索服务使用向量搜索。你可以同时使用 [Couchbase Capella](https://www.couchbase.com/products/capella/) 和你自管理的 Couchbase Server。

## 安装

你需要安装 couchbase 和 langchain community 来使用 couchbase 向量存储。在本教程中，我们将使用 OpenAI 嵌入。
...
我们在 `testing` 存储桶的 `_default` 作用域下的 `_default` 集合上定义一个索引，向量字段设置为 `embedding`，维度为 1536，文本字段设置为 `text`。
我们还对文档中 `metadata` 下的所有字段进行动态映射索引和存储，以适应不同的文档结构。相似度度量设置为 `dot_product`。
...
textKey 和 embeddingKey 是可选字段，如果你想使用特定键，则需要设置。
...
// 使用 OpenAIEmbeddings 需要 Open AI API 密钥，也可以使用其他一些嵌入
...
你可以在搜索期间使用过滤器中的 `fields` 参数指定要从文档返回的字段。
这些字段作为 `metadata` 对象的一部分返回。你可以获取索引中存储的任何字段。
文档的 `textKey` 作为文档 `pageContent` 的一部分返回。

如果你没有指定要获取的任何字段，则返回索引中存储的所有字段。

如果你想获取 metadata 中的一个字段，需要使用 `.` 来指定。
例如，要获取 metadata 中的 `source` 字段，需要使用 `metadata.source`。
...
Couchbase 允许你通过将向量搜索结果与文档非向量字段（如 `metadata` 对象）上的搜索相结合来进行混合搜索。

结果将基于向量搜索和全文搜索服务支持的搜索结果的组合。
每个组件搜索的分数相加得到结果的总分。

要执行混合搜索，可以在 `fields` 参数中有一个可选键 `searchOptions`，可以传递给所有相似性搜索。
你可以在 [Couchbase 搜索请求参数文档](https://docs.couchbase.com/server/current/search/search-request-params.html#query-object) 中找到 `searchOptions` 的不同搜索/查询可能性。

### 为混合搜索创建多样化的元数据

为了模拟混合搜索，让我们从现有文档中创建一些随机元数据。
我们均匀地向元数据添加三个字段：`date` 在 2010 到 2020 之间，`rating` 在 1 到 5 之间，`author` 设置为 John Doe 或 Jane Doe。
我们还将声明一些示例查询。
...
我们可以通过为搜索指定模糊度来搜索部分匹配。当你想搜索搜索查询的轻微变化或拼写错误时，这很有用。

这里，"Johny" 接近（模糊度为 1）"John Doe"。
...
我们可以搜索日期字段（如 `metadata.date`）在日期范围查询内的文档。
...
我们可以搜索数值字段（如 `metadata.rating`）在范围内的文档。
...
在 Couchbase 中，我们只能返回搜索索引中存储的字段。请确保你尝试在搜索结果中访问的字段是搜索索引的一部分。

处理此问题的一种方法是在索引中动态索引和存储文档的字段。

- 在 Capella 中，你需要进入“高级模式”，然后在“常规设置”下勾选“[X] 存储动态字段”或“[X] 索引动态字段”
...
这很可能是由于 Couchbase 搜索索引没有索引和/或存储文档中的 `metadata` 字段。为了索引文档中的 `metadata` 字段，你需要将其作为子映射添加到索引中。

如果你选择映射映射中的所有字段，你将能够按所有元数据字段进行搜索。或者，为了优化索引，你可以选择索引 `metadata` 对象内的特定字段。
你可以参考[文档](https://docs.couchbase.com/cloud/search/customize-index.html)来了解有关索引子映射的更多信息。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\couchbase_search.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
