...
本指南提供了快速入门 MariaDB [向量存储](/oss/javascript/integrations/vectorstores) 的概述。有关 `MariaDBStore` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/mariadb/MariaDBStore)。
...
| :--- | :--- | :---: | :---: |
| [`MariaDBStore`](https://reference.langchain.com/javascript/langchain-community/vectorstores/mariadb/MariaDBStore) | [`@langchain/community`](https://npmjs.com/@langchain/community) | ✅ | ![NPM - 版本](https://img.shields.io/npm/v/@langchain/community?style=flat-square&label=%20&) |

## 设置

要使用 MariaDBVector 向量存储，你需要设置 MariaDB 11.7 或更高版本，并将 [`mariadb`](https://www.npmjs.com/package/mariadb) 连接器作为对等依赖项。
...
如果你想获取模型调用的自动跟踪，也可以通过取消注释以下内容来设置你的 [LangSmith](/langsmith/home) API 密钥：
...
要实例化向量存储，请调用 `.initialize()` 静态方法。这将自动检查 `config` 中给定的 `tableName` 表是否存在。如果不存在，它将创建具有所需列的表。
...
一旦你的向量存储创建完成并添加了相关文档，你很可能希望在运行链或代理时查询它。
...
```typescript
const similaritySearchResults = await vectorStore.similaritySearch("生物学", 2, { "year": 2021 });
...
let res = await vectorStore.similaritySearch("生物学", 2, {"$or": [{"name":"martin"}, {"firstname", "john"}] });
```
...
```text
* [SIM=0.835] 细胞的核心是线粒体 [{"source":"https://example.com"}]
...
你可以通过创建连接池，然后直接通过构造函数创建新的 `MariaDBStore` 实例来重用连接。

注意，你应该至少调用一次 `.initialize()` 来正确设置数据库表，然后再使用构造函数。
...
const results = await mariadbStore.similaritySearch("水", 1);
...
const results2 = await mariadbStore2.similaritySearch("水", 1);
...
有关 `MariaDBStore` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/mariadb/MariaDBStore)。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\mariadb.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
