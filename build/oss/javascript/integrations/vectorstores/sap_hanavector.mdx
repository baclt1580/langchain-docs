...
确保您的 SAP HANA 实例正在运行。从环境变量加载凭证并使用您首选的 HANA 客户端创建连接。
...
了解有关 SAP HANA 的更多信息，请参见 [什么是 SAP HANA？](https://www.sap.com/products/data-cloud/hana/what-is-sap-hana.html)。
...
或者，您可以直接使用 SAP HANA 的原生 `VECTOR_EMBEDDING()` 函数计算嵌入。如果您的 TypeScript 环境中可用内部嵌入支持，请类似地初始化和传递给 `HanaDB`。有关内部嵌入的更多信息，请参见 [SAP HANA VECTOR_EMBEDDING 函数](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/vector-embedding-function-vector)。
...
// 可选地，您可以指定远程源以使用已部署的 SAP AI CORE 实例中的模型：
/*
const internalEmbeddings = new HanaInternalEmbeddings({
  internalEmbeddingModelId:
...
我们可以使用 `addDocuments` 函数将项添加到我们的向量存储中。
...
您可以通过使用 `distanceStrategy` 参数在初始化 `HanaDB` 实例时指定距离策略。
...
向量索引可以显著加快向量的 top-k 最近邻查询。用户可以使用 `createHnswIndex` 函数创建分层可导航小世界 (HNSW) 向量索引。
...
- [方法：使用 RAG 问答](https://js.langchain.com/docs/how_to/#qa-with-rag)
...
由于 HANA 的 dbapi 驱动程序默认将向量列输出为 Buffer 对象，我们将创建一个辅助函数将该函数转换为数字列表。
...
- 用于嵌入文本/上下文的 `NCLOB` 或 `NVARCHAR` 类型列
- 用于元数据的 `NCLOB` 或 `NVARCHAR` 类型列
- 用于嵌入向量的 `REAL_VECTOR` 或 `HALF_VECTOR` 类型列

表可以包含额外的列。当新文档插入表时，这些额外列必须允许 NULL 值。
...
为了允许灵活的元数据值，所有元数据默认都存储在元数据列中的 JSON 中。如果已知某些使用的元数据键和值类型，则可以通过创建具有键名为列名的目标表并将它们通过 `specificMetadataColumns` 列表传递给 HanaDB 构造函数来将它们存储在额外的列中。匹配这些值的元数据键在插入期间被复制到特殊列中。过滤器使用特殊列而不是元数据 JSON 列来处理 `specificMetadataColumns` 列表中的键。
...
特殊列对 langchain 接口的其余部分完全透明。一切按原样工作，只是性能更高。
...
执行查询以获取在上一步中添加的文档块中匹配度最高的两个文档块。
默认情况下，搜索使用 "Cosine Similarity"。
...
使用 "Euclidean Distance" 查询相同的内容。结果应与 "Cosine Similarity" 相同。
...
`Maximal marginal relevance` 针对与查询的相似性和所选文档之间的多样性进行优化。前 20 个 (fetch_k) 项将从数据库中检索。然后 MMR 算法将找到最佳 2 个 (k) 匹配项。
...
- **默认参数**：在 `createHnswIndex` 函数中，如果用户未提供 `m`、`efConstruction` 或 `efSearch` 等参数的自定义值，将自动使用默认值（例如 `m=64`、`efConstruction=128`、`efSearch=200`）。这些值确保在不要求用户干预的情况下以合理的性能创建索引。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\sap_hanavector.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
