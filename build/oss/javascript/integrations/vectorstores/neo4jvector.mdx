---
title: "Neo4j 向量索引集成"
description: "使用 LangChain JavaScript 与 Neo4j 向量索引向量存储集成。"
---

Neo4j 是一个开源图数据库，内置支持向量相似性搜索。
它支持：
...
你可以在 Neo4j [网站](https://neo4j.com/docs/operations-manual/current/installation/)上找到更多关于如何设置 `Neo4j` 的信息。
...
/*
 * retrievalQuery 是一个可自定义的 Cypher 查询片段，用于在 Neo4jVectorStore 类中定义如何从 Neo4j 数据库检索和呈现搜索结果。
 ...
 *
 * 3. metadata:
 *    - 描述：包含节点的其他属性和信息。可以包括节点任何其他可能与应用程序相关的属性。
 *    - 类型：对象（键值对）
 *    - 示例：{ "id": "12345", "category": "Books", "author": "John Doe" }
 *
 * 注意：虽然你可以自定义 `retrievalQuery` 以获取额外的列或执行转换，但绝不能省略必需列。
 ...
 * 它设计用于处理已有文本属性但可能没有嵌入向量的节点。
 * 该方法将为缺少嵌入向量的节点计算并存储嵌入向量。
 *
 * 注意：
 * 当你有一个包含文本数据的预存图，并且希望在不改变原始数据结构的情况下通过向量嵌入增强其相似性搜索能力时，此方法特别有用。
 */
 ...
 *
 * 描述：
 * 此方法在 Neo4j 向量索引中实现高级相似性搜索，同时利用文本嵌入和元数据属性。
 * 第三个参数 `filter` 允许指定基于元数据的条件，在执行相似性搜索之前预过滤节点。
 * 这种方法通过允许用户基于复杂的元数据标准以及文本相似性进行查询，提高了搜索精度。
 * 元数据过滤还支持以下运算符：
 *
 *  $eq: 等于
 *  $ne: 不等于
 ...
 *  $ilike: 小写文本包含值
 *
 * 过滤器支持一系列查询操作，如相等检查、范围查询和复合条件（使用逻辑运算符如 $and、$or）。
 * 这使得它高度适应各种用例，需要基于内容和上下文信息详细且特定地检索文档。
 *
 * 注意：
 * 有效使用此方法需要一个结构良好的 Neo4j 数据库，其中节点同时包含文本和元数据属性。
 * 在文本分析与详细元数据查询的集成至关重要的场景中，此方法特别有用，例如内容推荐系统、详细的档案搜索或任何上下文相关性是关键的应用。
 */
...
_安全提示_：请确保数据库连接使用的凭据权限范围尽可能小，仅包含必要的权限。
否则可能导致数据损坏或丢失，因为调用代码可能会尝试执行删除、数据突变（如果被适当触发）或读取敏感数据（如果数据库中存在此类数据）。
防范此类负面结果的最佳方法是（根据情况）限制授予此工具使用的凭据的权限。
例如，为数据库创建只读用户是确保调用代码无法突变或删除数据的好方法。
