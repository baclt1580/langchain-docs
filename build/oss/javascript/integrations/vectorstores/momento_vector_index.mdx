...
description: "使用 LangChain JavaScript 与 Momento 向量索引 (MVI) 向量存储集成。"
---

[MVI](https://gomomento.com)：为您的数据提供最高效、最易用、无服务器的向量索引。要开始使用 MVI，只需注册一个账户。无需处理基础设施、管理服务器或担心扩展问题。MVI 是一项能自动扩展以满足您需求的服务。无论是在 Node.js、浏览器还是边缘环境中，Momento 都能为您提供支持。
...
`ids` 是可选的；如果省略，Momento 将为您生成 UUID。
...
使用 `fromDocuments` 可以让您将各种文档加载器与索引无缝链接。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\vectorstores\momento_vector_index.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
