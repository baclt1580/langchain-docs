---
title: "Layerup 安全集成"
description: "使用 LangChain JavaScript 集成 Layerup 安全 LLM。"
---
...
接下来，通过 [仪表板](https://dashboard.uselayerup.com) 创建一个项目，并复制您的 API 密钥。我们建议将 API 密钥放在项目的环境变量中。
...
const layerupSecurityOptions: LayerupSecurityOptions = {
  ...
  // 是否在将提示发送给 LLM 之前屏蔽其中的 PII 和敏感数据
  mask: false,
  ...
  handlePromptGuardrailViolation: (violation: GuardrailResponse) => {
    ...
    return {
      role: "assistant",
      content: `存在敏感数据！我无法回应。这是一个动态的预设回复。当前日期：${Date.now()}`,
    };
  },
  ...
  handleResponseGuardrailViolation: (violation: GuardrailResponse) => ({
    role: "assistant",
    content: `带有动态数据的自定义预设回复！违规规则是 ${violation.offending_guardrail}。`,
  }),
};
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\llms\layerup_security.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
