...
LangChain 集成了 PromptLayer，用于记录和调试提示与响应。要添加 PromptLayer 支持：

1. 在此处创建 PromptLayer 账户：[https://promptlayer.com](https://www.promptlayer.com/)。
2. 创建 API 令牌，并将其作为 `promptLayerApiKey` 参数传入 `PromptLayerOpenAI` 构造函数，或设置在 `PROMPTLAYER_API_KEY` 环境变量中。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\llms\prompt_layer_openai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
