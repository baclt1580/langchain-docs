...
在摄取 HTML 文档以供后续检索时，我们通常只关注网页的实际内容，而非其语义结构。
使用 HtmlToTextTransformer 剥离文档中的 HTML 标签可以生成内容更丰富的文本块，从而提高检索效果。
...
```
虽然转换器本身不需要，但以下使用示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 进行网页抓取：
...
以下示例抓取了一个 Hacker News 帖子，根据 HTML 标签进行分割以基于标签语义信息分组文本块，
然后从各个块中提取内容：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\document_transformers\html-to-text.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
