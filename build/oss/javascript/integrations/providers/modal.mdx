...
title: "Modal"
sidebarTitle: "Modal"
description: "使用 Modal 沙盒后端配合 deepagents，实现支持 GPU 的隔离代码执行环境"
---
...
try {
  const agent = createDeepAgent({
    ...
    systemPrompt: "您是一个拥有沙盒访问权限的编程助手。",
    backend: sandbox,
  });

  const result = await agent.invoke({
    messages: [{ role: "user", content: "安装 numpy 并计算 pi" }],
  });
} finally {
  await sandbox.close();
}
...
| `volumes` | `Record<string, string>` | - | 卷映射（挂载路径到卷名） |
...
挂载 Modal Volumes 以实现持久化存储，并将密钥作为环境变量注入：
...
对于 `BaseSandbox` 未暴露的高级功能，可以访问底层的 Modal SDK：
  ...
  if (error instanceof ModalSandboxError) {
    switch (error.code) {
      ...
      case "AUTHENTICATION_FAILED":
        console.error("请检查您的 Modal 令牌凭据");
        break;
    }
  }
}
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\providers\modal.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
