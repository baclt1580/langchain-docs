...
description: "将 Node.js VFS 沙盒后端与 deepagents 结合使用，用于本地开发和测试"
---

VFS 沙盒完全在本地运行，使用内存中的虚拟文件系统。无需云服务、Docker 或外部依赖——非常适合开发和测试。

它使用了 [node-vfs-polyfill](https://github.com/vercel-labs/node-vfs-polyfill)，该库实现了即将推出的 Node.js VFS 功能 ([nodejs/node#61478](https://github.com/nodejs/node/pull/61478))。
...
try {
  const agent = createDeepAgent({
    ...
    systemPrompt: "你是一个拥有 VFS 访问权限的编程助手。",
    backend: sandbox,
  });

  const result = await agent.invoke({
    messages: [{ role: "user", content: "运行 index.js 文件" }],
  });
} finally {
  await sandbox.stop();
}
...
| `initialFiles` | `Record<string, string \| Uint8Array>` | - | 用于初始化 VFS 的初始文件 |
...
2.  **命令执行**：执行命令时，文件同步到临时目录，命令运行，然后将更改同步回 VFS
3.  **回退模式**：如果 node-vfs-polyfill 不可用，则回退到使用临时目录进行存储和执行

这提供了内存存储（隔离性、速度）的优势，同时保持了完整的 shell 命令执行支持。
...
本包使用了 [node-vfs-polyfill](https://github.com/vercel-labs/node-vfs-polyfill)，它实现了正在 [nodejs/node#61478](https://github.com/nodejs/node/pull/61478) 中开发的即将推出的 Node.js VFS 功能。当官方的 `node:vfs` 模块在 Node.js 中落地时，本包将更新为使用原生实现。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\providers\node-vfs.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
