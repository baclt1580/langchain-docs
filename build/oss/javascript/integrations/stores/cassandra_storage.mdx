...
本示例演示了如何使用 `CassandraKVStore` [`BaseStore`](https://reference.langchain.com/javascript/langchain-core/stores/BaseStore) 集成来设置聊天历史存储。请注意，还有一个 `CassandraChatMessageHistory` 集成，可能更易于用于聊天历史存储；而 `CassandraKVStore` 适用于需要更通用的、支持前缀键的键值存储的场景。
...
```
根据您的数据库提供商，连接数据库的具体方式会有所不同。我们将创建一个文档 `configConnection`，它将作为存储配置的一部分使用。
...
// 此文档是 Cassandra 驱动程序的连接文档；示例针对 AstraDB，但
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\stores\cassandra_storage.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
