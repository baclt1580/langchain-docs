...
本笔记本提供了快速入门 `PDFLoader` [文档加载器](/oss/javascript/integrations/document_loaders) 的简要概述。有关 `PDFLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader)。
...
| :--- | :--- | :---: | :---: |  :---: |
| [`PDFLoader`](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader) | [`@langchain/community`](https://www.npmjs.com/package/@langchain/community) | 仅限 Node.js | ✅ | 🟠 (参见下方说明) |

## 设置

要使用 `PDFLoader` 文档加载器，您需要安装 `@langchain/community` 集成包以及 `pdf-parse` 包。
...
在以下示例中，我们使用 `pdfjs-dist` 的“传统”构建（参见 [pdfjs 文档](https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions#which-browsersenvironments-are-supported)），其中包含默认构建中未包含的多个 polyfill。
...
/* 额外步骤：使用任何 TextSplitter 将文本分割成块。然后您可以将其用作上下文或之后保存到内存中。 */
...
有关 PDFLoader 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/fs/pdf/PDFLoader)。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\document_loaders\file_loaders\pdf.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
