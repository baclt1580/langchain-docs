...
本笔记本提供了快速入门 `PuppeteerWebBaseLoader` [文档加载器](/oss/javascript/integrations/document_loaders/) 的概述。有关 `PuppeteerWebBaseLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/puppeteer/PuppeteerWebBaseLoader)。

Puppeteer 是一个 Node.js 库，提供了控制无头 Chrome 或 Chromium 的高级 API。您可以使用 Puppeteer 自动化网页交互，包括从需要 JavaScript 渲染的动态网页中提取数据。
...
如果您希望获取模型调用的自动跟踪，还可以通过取消注释以下内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
要对网站进行截图，请像上面一样初始化加载器，并调用 `.screenshot()` 方法。
...
有关 `PuppeteerWebBaseLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/document_loaders/web/puppeteer/PuppeteerWebBaseLoader)。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\document_loaders\web_loaders\web_puppeteer.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
