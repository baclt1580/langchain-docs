...
与基于关键词的搜索（如 Google）不同，Exa 的神经搜索能力使其能够语义化理解查询并返回相关文档。例如，我们可以搜索 `"关于猫的有趣文章"`，并比较 Google 和 Exa 的搜索结果。Google 会根据关键词 "fascinating" 给出 SEO 优化的列表文章。而 Exa 则能直接给出所需结果。
...
| :--- | :--- | :---: | :---: | :---: |
| [`ExaSearchResults`](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults) | [`@langchain/exa`](https://npmjs.com/package/@langchain/exa) | ❌ | ✅ |  ![NPM - 版本](https://img.shields.io/npm/v/@langchain/exa?style=flat-square&label=%20&) |
...
首先，获取 Exa API 密钥并将其添加为环境变量。通过[注册 Exa](https://dashboard.exa.ai/login) 每月可获得 1000 次免费搜索。

```typescript
process.env.EXASEARCH_API_KEY="your-api-key"
```

同时设置 LangSmith 以获得最佳的观测性（非必需但推荐）：
...
我们也可以使用模型生成的 `ToolCall` 来调用工具，此时将返回一个 [`ToolMessage`](https://reference.langchain.com/javascript/langchain-core/messages/ToolMessage)：
...
const toolChainResult = await toolChain("Anthropic 2024 年的预计收入是多少？");
```
...
我们可以创建使用 `ExaRetriever` 和 `createRetrieverTool` 的 LangChain 工具。使用这些工具，我们可以构建一个简单的搜索智能体，能够回答任何主题的问题。

我们将使用 LangGraph 创建智能体。确保已安装 `@langchain/langgraph`：
...
```typescript
const exampleQuery = "为我总结一篇关于猫的有趣文章。"

const events = await agentExecutor.stream(
  { messages: [
    [
      "system",
      `你是一个网络研究员，通过查找互联网信息并检索有用文档的内容来回答用户问题。请引用你的来源。`,
    ],
    ["human", exampleQuery],
  ] },
  { streamMode: "values", }
...
我找到了一篇来自《大西洋月刊》的有趣文章，深入探讨了猫咪护理中常被忽视的一个方面：给猫刷牙。尽管这是维护猫咪健康的关键部分，但很少有猫主人真正给宠物刷牙。文章强调了这项任务的挑战性和荒谬性，指出只有不到 5% 的猫主人进行这项实践。

兽医专家强调猫咪牙齿卫生的重要性，因为大多数四岁以上的家猫都会患上某种形式的牙龈疾病，如果不及时治疗，可能导致严重的健康问题。文章讨论了各种工具和技术，如禽肉味牙膏和猫用牙刷，这些可以使过程更容易。然而，文章也承认了许多猫对刷牙表现出的困难和抗拒。

有趣的是，文章还涉及了宠物护理的更广泛背景，指出围绕宠物维护的期望和实践如何随着时间的推移而演变。它建议，虽然给猫刷牙是理想的，但像洁牙零食和专用猫粮这样的替代方法也可能有效。

更多详情，请阅读[《大西洋月刊》全文](https://www.theatlantic.com/health/archive/2023/10/cat-dental-care-toothbrushing/675123/)。
```
...
有关 `ExaSearchResults` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-exa/ExaSearchResults)

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\tools\exa_search.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
