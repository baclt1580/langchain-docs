---
title: 使用 `HanaSparqlQAChain` 进行问答
---

## 安装与设置

要使用此功能，请安装 `@sap/hana-langchain` 包及其对等依赖项：

```bash
npm install @sap/hana-langchain @langchain/core@latest @langchain/classic@latest langchain@latest
```

创建到您的 SAP HANA Cloud 实例的连接。

```typescript
import * as dotenv from 'dotenv';
// 如果需要，加载环境变量
dotenv.config();

import hanaClient from "@sap/hana-client";

const connectionParams = {
  host: process.env.HANA_DB_ADDRESS,
  port: process.env.HANA_DB_PORT,
  user: process.env.HANA_DB_USER,
  password: process.env.HANA_DB_PASSWORD,
};
const client = hanaClient.createConnection(connectionParams);

// 连接到 hanaDB
await new Promise<void>((resolve, reject) => {
  client.connect((err: Error) => {
    // 此处使用箭头函数
    if (err) {
      reject(err);
    } else {
      console.log("成功连接到 SAP HANA。");
      resolve();
    }
  });
});
```

`HanaSparqlQAChain` 将以下部分整合在一起：

1. **模式感知的 SPARQL 生成**
2. 针对 SAP HANA 的**查询执行**
3. **自然语言答案格式化**

## 初始化

* 一个用于生成和解释查询的 **LLM**
* 一个 **`HanaRdfGraph`**（包含连接、`graph_uri` 和本体）

请按照 [HanaRdfGraph](/oss/javascript/integrations/graphs/sap_hana_rdf_graph) 中的步骤了解更多关于创建 `HanaRdfGraph` 实例的信息。

导入 `HanaSparqlQAChain`

```typescript
import { HanaSparqlQAChain } from "@sap/hana-langchain";
const qaChain = HanaSparqlQAChain.fromLLM({ llm, graph, allowDangerousRequests: true, verbose: true });
```

## 流程概述

1. SPARQL 生成
   - 使用 SPARQL 生成提示
   - 输入：`schema`（来自 `graph.getSchema()` 的 Turtle 格式）、`prompt`（用户问题）
2. 查询后处理
   - 从模型输出中提取 SPARQL
   - 如果缺失，则注入 `FROM <graph_uri>`
   - 确保常用前缀（`rdf:`、`rdfs:`、`owl:`、`xsd:`）
3. 执行
   - `graph.query(generatedSparql)`
4. 答案构建
   - 使用 QA 提示，输入：`context`（结果）、`prompt`（原始问题）

## 提示模板

### "SPARQL 生成" 提示

`sparqlGenerationPrompt` 用于引导 LLM 根据用户问题和提供的模式生成 SPARQL 查询。

### 答案生成提示

`qaPrompt` 指示 LLM 仅基于数据库结果创建自然语言答案。
默认提示可以在这里找到：[`prompts.ts`](https://github.com/SAP/langchainjs-integration-for-sap-hana-cloud/blob/main/src/chains/graph_qa/prompts.ts)

## 自定义提示

您可以在初始化时覆盖默认值：

```typescript
const qaChain = HanaSparqlQAChain.fromLLM({
  llm,
  graph,
  allowDangerousRequests: true,
  verbose: true,
  sparqlGenerationPrompt: YOUR_SPARQL_PROMPT,
  qaPrompt: YOUR_QA_PROMPT,
});
```

> - `sparql_generation_prompt` 必须包含输入变量：`["schema", "prompt"]`
> - `qa_prompt` 必须包含输入变量：`["context", "prompt"]`

## 示例：在“电影”知识图谱上进行问答

**前提条件**：
您必须拥有一个启用了**三元组存储**功能的 SAP HANA Cloud 实例。
详细说明请参考：[启用三元组存储](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/enable-triple-store/)<br />
加载 `kgdocu_movies` 示例数据。请参阅[知识图谱示例](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/knowledge-graph-example)。

下面我们将：

1. 实例化指向“电影”数据图谱的 `HanaRdfGraph`
2. 将其包装在由 LLM 驱动的 `HanaSparqlQAChain` 中
3. 提出自然语言问题并打印出链的响应

这演示了 LLM 如何在后台生成 SPARQL，针对 SAP HANA 执行，并返回人类可读的答案。

首先，创建到您的 SAP HANA Cloud 实例的连接。

```typescript
import * as dotenv from 'dotenv';
// 如果需要，加载环境变量
dotenv.config();

import hanaClient from "@sap/hana-client";

const connectionParams = {
  host: process.env.HANA_DB_ADDRESS,
  port: process.env.HANA_DB_PORT,
  user: process.env.HANA_DB_USER,
  password: process.env.HANA_DB_PASSWORD,
};
const client = hanaClient.createConnection(connectionParams);

// 连接到 hanaDB
await new Promise<void>((resolve, reject) => {
  client.connect((err: Error) => {
    // 此处使用箭头函数
    if (err) {
      reject(err);
    } else {
      console.log("成功连接到 SAP HANA。");
      resolve();
    }
  });
});
```

然后，设置知识图谱实例

```typescript
import { HanaRdfGraph, HanaRdfGraphOptions } from "@sap/hana-langchain";

const graphOptions: HanaRdfGraphOptions = {
  connection: client,
  graphUri: "kgdocu_movies",
  autoExtractOntology: true,
};

// 从源 URI 创建 Graph 实例
const graph = new HanaRdfGraph(graphOptions);

// 实例创建后需要初始化一次。
await graph.initialize(graphOptions);
```

```typescript
// 序列化图谱模式（可选）
// 在内部，模式存储为 N3 Store 实例，
// 我们使用 N3 Writer 将其序列化为 Turtle 格式以便显示。
const schemaStore = graph.getSchema();
const writer = new Writer({
  prefixes: {
    rdf: "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
    rdfs: "http://www.w3.org/2000/01/rdf-schema#",
    owl: "http://www.w3.org/2002/07/owl#",
    xsd: "http://www.w3.org/2001/XMLSchema#",
  },
});
schemaStore.forEach((quad) => {
  writer.addQuad(quad);
});
writer.end((error, result) => {
  if (error) {
    console.error("序列化模式时出错：", error);
  } else {
    console.log("Turtle 格式的图谱模式：\n", result);
  }
});
```

```text
Turtle 格式的图谱模式：
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>.
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#>.
@prefix owl: <http://www.w3.org/2002/07/owl#>.
@prefix xsd: <http://www.w3.org/2001/XMLSchema#>.

<http://kg.demo.sap.com/Place> a owl:Class;
    rdfs:label "Place".
rdfs:label a owl:DatatypeProperty;
    rdfs:label "label";
    rdfs:domain <http://kg.demo.sap.com/Place>, <http://kg.demo.sap.com/Actor>, <http://kg.demo.sap.com/Director>, <http://kg.demo.sap.com/Genre>;
    rdfs:range xsd:string.
<http://kg.demo.sap.com/Actor> a owl:Class;
    rdfs:label "Actor".
<http://kg.demo.sap.com/Film> a owl:Class;
    rdfs:label "Film".
<http://kg.demo.sap.com/Director> a owl:Class;
    rdfs:label "Director".
<http://kg.demo.sap.com/Genre> a owl:Class;
    rdfs:label "Genre".
<http://kg.demo.sap.com/dateOfBirth> a owl:DatatypeProperty;
    rdfs:label "dateOfBirth";
    rdfs:domain <http://kg.demo.sap.com/Actor>;
    rdfs:range xsd:dateTime.
<http://kg.demo.sap.com/placeOfBirth> a owl:ObjectProperty;
    rdfs:label "placeOfBirth";
    rdfs:domain <http://kg.demo.sap.com/Actor>;
    rdfs:range <http://kg.demo.sap.com/Place>.
<http://kg.demo.sap.com/title> a owl:DatatypeProperty;
    rdfs:label "title";
    rdfs:domain <http://kg.demo.sap.com/Film>;
    rdfs:range xsd:string.
<http://kg.demo.sap.com/directed> a owl:ObjectProperty;
    rdfs:label "directed";
    rdfs:domain <http://kg.demo.sap.com/Director>;
    rdfs:range <http://kg.demo.sap.com/Film>.
<http://kg.demo.sap.com/acted_in> a owl:ObjectProperty;
    rdfs:label "acted_in";
    rdfs:domain <http://kg.demo.sap.com/Actor>;
    rdfs:range <http://kg.demo.sap.com/Film>.
<http://kg.demo.sap.com/genre> a owl:ObjectProperty;
    rdfs:label "genre";
    rdfs:domain <http://kg.demo.sap.com/Film>;
    rdfs:range <http://kg.demo.sap.com/Genre>.
```

之后，初始化 LLM。

```typescript
import { ChatOpenAI } from "@langchain/openai"; // 或您选择的 LLM
const llm = new ChatOpenAI({ model: "gpt-4o" });
```

然后，我们创建一个 SPARQL QA 链

```typescript
import { HanaSparqlQAChain, HanaSparqlQAChainOptions } from "@sap/hana-langchain";

const chainOptions: HanaSparqlQAChainOptions = {
  llm,
  allowDangerousRequests: true,
  graph,
  debug: true,
};

const query = "哪些演员出演了《银翼杀手》？";
// const query = "数据中有哪些电影？"
// const query = "基努·里维斯和凯瑞-安·莫斯一起出演了哪些电影？"
// const query = "数据中有哪些电影类型？"
// const query = "哪两种电影类型被分配得最多？"
// const query = "《银翼杀手》的演员出生在哪里？"
// const query = "哪些演员在同一部电影中合作并且出生在同一城市？"

const response = await chain.invoke({ query });
console.log(response["result"]);
```

```text
生成的 SPARQL：
\`\`\`sparql
PREFIX kg: <http://kg.demo.sap.com/>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT ?actor ?actorLabel
WHERE {
    ?movie rdf:type kg:Film .
    ?movie kg:title "Blade Runner" .
    ?actor kg:acted_in ?movie .
    ?actor rdfs:label ?actorLabel .
}
\`\`\`
最终 SPARQL：


PREFIX kg: <http://kg.demo.sap.com/>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT ?actor ?actorLabel

      FROM <kgdocu_movies>
      WHERE {
    ?movie rdf:type kg:Film .
    ?movie kg:title "Blade Runner" .
    ?actor kg:acted_in ?movie .
    ?actor rdfs:label ?actorLabel .
}


完整上下文：
actor,actorLabel
http://www.wikidata.org/entity/Q1353691,Morgan Paull
http://www.wikidata.org/entity/Q1372770,William Sanderson
http://www.wikidata.org/entity/Q358990,James Hong
http://www.wikidata.org/entity/Q723780,Brion James
http://www.wikidata.org/entity/Q81328,Q81328
http://www.wikidata.org/entity/Q498420,M. Emmet Walsh
http://www.wikidata.org/entity/Q1691628,Joe Turkel
http://www.wikidata.org/entity/Q207596,Daryl Hannah
http://www.wikidata.org/entity/Q236702,Joanna Cassidy
http://www.wikidata.org/entity/Q213574,Rutger Hauer
http://www.wikidata.org/entity/Q3143555,Hy Pyke
http://www.wikidata.org/entity/Q230736,Sean Young
http://www.wikidata.org/entity/Q211415,Edward James Olmos

出演《银翼杀手》的演员包括 Morgan Paull、William Sanderson、James Hong、Brion James、M. Emmet Walsh、Joe Turkel、Daryl Hannah、Joanna Cassidy、Rutger Hauer、Hy Pyke、Sean Young 和 Edward James Olmos。
```

## 底层发生了什么？

1. **SPARQL 生成**
   该链使用 `SPARQL_GENERATION_SELECT_PROMPT`，调用 LLM 并传入您的 Turtle 格式本体（`graph.getSchema()`）和用户问题。然后 LLM 生成一个针对您模式定制的有效 `SELECT` 查询。
2. **预处理与执行**
   * **提取与清理**：从 LLM 的响应中提取原始 SPARQL 文本。
   * **注入图谱上下文**：如果缺失，则添加 `FROM <graph_uri>`，并确保声明了常用前缀（`rdf:`、`rdfs:`、`owl:`、`xsd:`）。
   * **在 HANA 上运行**：通过 `HanaRdfGraph.query()` 在您的命名图谱上执行最终查询。
3. **答案构建**
   返回的 CSV（或 Turtle）结果再次输入到 LLM 中——这次使用 `SPARQL_QA_PROMPT`。LLM 根据检索到的数据生成一个简洁、人类可读的答案，不会产生幻觉。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\chains\sap_hana_sparql_qa_chain.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
<Callout icon="terminal-2">
    [Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.
</Callout>
</div>
