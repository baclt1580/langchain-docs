...
本文将帮助您开始使用 LangChain 的 OpenAIEmbeddings [嵌入模型](/oss/javascript/integrations/embeddings)。关于 `OpenAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-openai/OpenAIEmbeddings)。
...
下面，我们将使用上面初始化的 `embeddings` 对象演示如何索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
对于 `text-embedding-3` 系列的模型，您可以指定返回的嵌入维度。例如，默认情况下 `text-embedding-3-large` 返回维度为 3072 的嵌入：
...
通过传入 `dimensions: 1024`，我们可以将嵌入的维度减少到 1024：
...
如果您在 Azure OpenAI 上托管，请参阅 [专用页面](/oss/javascript/integrations/embeddings/azure_openai)。

---

## API 参考

关于所有 `OpenAIEmbeddings` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-openai/OpenAIEmbeddings)。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\integrations\embeddings\openai.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
