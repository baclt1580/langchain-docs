...
| [模型前钩子](#模型前钩子) | 替换为带有 `beforeModel` 方法的中间件 |
| [模型后钩子](#模型后钩子) | 替换为带有 `afterModel` 方法的中间件 |
...
| [命名空间](#简化包) | 精简为专注于智能体构建块，遗留代码移至 `@langchain/classic` |
...
const agent = createReactAgent({
  model,
  tools,
  prompt: new SystemMessage(content: "你是一个乐于助人的助手。"), // [!code highlight]
});
```
</CodeGroup>
...
标准内容块默认**不会**序列化到 `content` 属性中。如果你需要在 `content` 属性中访问标准内容块（例如，向客户端发送消息时），可以选择将它们序列化到 `content` 中。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\oss\javascript\migrate\langchain-v1.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
