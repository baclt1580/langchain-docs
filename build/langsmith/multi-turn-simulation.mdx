...
具有对话界面的 AI 应用（如聊天机器人）需要与用户进行多次交互，也称为对话*轮次*。在评估此类应用的性能时，核心概念如[构建数据集](/langsmith/evaluation-concepts#datasets)和定义[评估器](/langsmith/evaluation-concepts#evaluators)及指标来评判应用输出仍然有用。然而，您可能还会发现，在您的应用和用户之间运行一次*模拟*，然后评估这个动态创建的轨迹也很有帮助。
...
缺点是，由于您将评估范围扩大到包含多个轮次，因此相比基于数据集的静态输入评估应用的单一输出，一致性会降低。

![多轮轨迹](/langsmith/images/multi-turn-trace.png)

本指南将展示如何使用开源的 [`openevals`](https://github.com/langchain-ai/openevals) 包来模拟多轮交互并进行评估。该包包含预构建的评估器和其他用于评估 AI 应用的便捷资源。本指南也将使用 OpenAI 模型，不过您也可以使用其他供应商的模型。
...
* `user`：模拟用户。在本指南中，我们将使用一个名为 `create_llm_simulated_user` 的预构建导入函数，该函数使用 LLM 生成用户响应，不过您也可以[创建自己的模拟用户](https://github.com/langchain-ai/openevals?tab=readme-ov-file#custom-simulated-users)。
...
模拟首先从模拟的 `user` 生成初始查询，然后在 `app` 和 `user` 之间来回传递响应聊天消息，直到达到 `max_turns`（您也可以传递一个 `stopping_condition`，该函数接收当前轨迹并返回 `True` 或 `False` - [详见 OpenEvals README](https://github.com/langchain-ai/openevals?tab=readme-ov-file#multiturn-simulation)）。返回值是构成对话**轨迹**的最终聊天消息列表。

<Info>
有多种方式可以配置模拟用户，例如让它在模拟的前几轮返回固定响应，以及整体模拟的配置。完整详情请查看 [OpenEvals README](https://github.com/langchain-ai/openevals?tab=readme-ov-file#multiturn-simulation)。
</Info>

最终的追踪将类似于[这样](https://smith.langchain.com/public/648ca37d-1c4d-4f7b-9b6a-89e35dc5d4f0/r)，其中包含您的 `app` 和 `user` 的交替响应：
...
您可以将多轮模拟的结果用作 LangSmith 实验的一部分，以跟踪性能并随时间推移观察进展。对于这些部分，熟悉至少一种 LangSmith 的 [`pytest`](/langsmith/pytest)（仅限 Python）、[`Vitest`/`Jest`](/langsmith/vitest-jest)（仅限 JS）或 [`evaluate`](/langsmith/evaluate-llm-application) 运行器会很有帮助。
...
如果您使用 [LangSmith 测试框架集成](/langsmith/pytest)之一，可以在运行模拟时将 OpenEvals 评估器数组作为 `trajectory_evaluators` 参数传入。这些评估器将在模拟结束时运行，将最终的聊天消息列表作为 `outputs` 关键字参数接收。因此，您传入的 `trajectory_evaluator` 必须接受此关键字参数。
...
LangSmith 将自动检测并记录从传入的 `trajectory_evaluators` 返回的反馈，并将其添加到实验中。还要注意，测试用例使用模拟用户的 `fixed_responses` 参数以特定输入开始对话，您可以记录该输入并将其作为存储数据集的一部分。
...
上述示例对所有输入示例使用相同的模拟用户角色，该角色由传递给 `create_llm_simulated_user` 的 `system` 参数定义。如果您希望为数据集中的特定项目使用不同的角色，可以更新数据集示例以包含一个带有所需 `system` 提示的额外字段，然后在创建模拟用户时像这样传入该字段：
...
* [在不同追踪中追踪多轮对话](/langsmith/threads)
* [在 Playground UI 中使用多条消息](/langsmith/multiple-messages)
* [在一个评估器中返回多个指标](/langsmith/multiple-scores)

您也可以探索 [OpenEvals 自述文件](https://github.com/langchain-ai/openevals)以获取更多关于预构建评估器的信息。

---

<div className="source-links">
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\multi-turn-simulation.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
