...
当您使用 Python 或 TypeScript SDK 的 `evaluate` 运行实验时，可以通过 `read_project`/`readProject` 方法获取实验的性能指标。
...
* `latency_p50`：第50百分位延迟（秒）。
* `latency_p99`：第99百分位延迟（秒）。
...
* `error_rate`：实验的错误率。
* `first_token_p50`：生成第一个令牌的第50百分位延迟（如果使用流式传输）。
* `first_token_p99`：生成第一个令牌的第99百分位延迟（如果使用流式传输）。

以下是使用 Python 和 TypeScript SDK 获取实验性能指标的示例。

首先，作为前提条件，我们将创建一个简单的数据集。这里我们仅用 Python 演示，但您也可以在 TypeScript 中实现相同操作。更多详情请查看评估的[操作指南](/langsmith/evaluate-llm-application)。
...
接下来，我们将创建一个实验，从 `evaluate` 的结果中获取实验名称，然后获取实验的性能指标。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\fetch-perf-metrics-experiment.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
