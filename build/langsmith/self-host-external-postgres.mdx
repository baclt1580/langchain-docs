---
title: 连接外部 PostgreSQL 数据库
sidebarTitle: 连接外部 PostgreSQL 数据库
---

LangSmith 使用 PostgreSQL 数据库作为事务性工作负载和操作数据（除了运行记录之外的几乎所有数据）的主要数据存储。默认情况下，LangSmith 自托管版将使用内部 PostgreSQL 数据库。但是，您可以配置 LangSmith 以使用外部 PostgreSQL 数据库。通过配置外部 PostgreSQL 数据库，您可以更轻松地管理数据库的备份、扩展和其他操作任务。

<Tip>
**如果您使用托管的 PostgreSQL 服务**，我们推荐：
...
* 一个已部署的 PostgreSQL 数据库，您的 LangSmith 实例需要能够通过网络访问它。我们建议使用托管的 PostgreSQL 服务，例如：
...
* 我们支持密码认证和 [IAM/工作负载身份](#iam-authentication) 认证。
...
* 默认情况下，我们推荐一个**至少具有 2 个 vCPU 和 8GB 内存**的实例。但是，实际要求将取决于您的工作负载和用户数量。我们建议监控您的 PostgreSQL 实例并根据需要进行扩展。
...
* 数据库
* 用户名
* 密码（如果有任何特殊字符，请确保进行 URL 编码）- **注意：** 使用 IAM 认证时，连接字符串中不需要密码。详见下文。
* URL 参数

其格式如下：

```
用户名:密码@主机:端口/数据库?<url参数>
```
...
准备好连接字符串后，您可以配置您的 LangSmith 实例以使用外部 PostgreSQL 数据库。您可以通过修改 LangSmith Helm Chart 安装的 `values` 文件或 Docker 安装的 `.env` 文件来实现。
...
postgres:
  external:
    enabled: true
    connectionUrl: "您的连接URL"
```

```bash Docker
# 在您的 .env 文件中
POSTGRES_DATABASE_URI="您的连接URL"
```
...
使用本节配置 PostgreSQL 连接的 TLS。关于挂载内部/公共 CA 以使 LangSmith 信任您的 PostgreSQL 服务器证书，请参阅 [配置自定义 TLS 证书](/langsmith/self-host-custom-tls-certificates#mount-internal-cas-for-tls)。
...
如果需要更精细的控制，请将 `fsGroup` 单独添加到每个 pod 的安全上下文中。有关完整参考，请参阅 [mTLS 配置示例](https://github.com/langchain-ai/helm/blob/main/charts/langsmith/examples/mtls_config.yaml)。

## IAM 认证

自 LangSmith helm chart 版本 **0.12.34** 起，我们支持 PostgreSQL 的 IAM 认证。这允许您使用云服务商的工作负载身份代替静态密码。

<Warning>
IAM 认证仅处理连接认证。您可能仍然需要在数据库中运行 SQL 命令来创建 IAM 用户/角色，并授予其访问 LangSmith 模式所需的权限和特权。
</Warning>
...
Amazon RDS 支持 [IAM 数据库认证](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.html)，允许您使用 AWS IAM 凭证而不是数据库密码来验证 PostgreSQL 实例。
...
2. 在您的 RDS PostgreSQL 实例上**启用 IAM 认证**并向您的工作负载身份授予访问权限

#### 配置

<Warning>
如果在 LangSmith 已运行初始迁移后切换到新的 IAM 用户，您可能需要将现有表的所有权转移给新的 IAM 用户。否则，由于先前用户拥有的表权限不足，迁移可能会失败。
</Warning>
...
<Warning>
IAM 认证需要 TLS。您必须在连接字符串中包含 `sslmode=require`。
</Warning>

#### 必需的注解

您必须将 AWS IRSA 所需的 ServiceAccount 注解应用到所有连接到 PostgreSQL 的 LangSmith 组件：
...
<Note>
上面列出的所有作业（除了 `e2eTest`）都使用 `backend` 服务账户。`e2eTest` 作业使用自己的服务账户，需要单独的注解配置。
</Note>
...
Cloud SQL 支持 [IAM 认证](https://cloud.google.com/sql/docs/postgres/iam-authentication)，允许您使用 GCP 服务账户而不是数据库密码进行认证。
...
2. 在您的 Cloud SQL 实例上**启用 IAM 认证**并向您的工作负载身份授予访问权限
...
您必须将 GCP Workload Identity 所需的 ServiceAccount 注解应用到所有连接到 PostgreSQL 的 LangSmith 组件：
...
### Azure Database for PostgreSQL 与 Microsoft Entra 认证

Azure Database for PostgreSQL 支持 [Microsoft Entra 认证](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-azure-ad-authentication)，允许您使用 Azure 托管标识而不是数据库密码进行认证。
...
2. 在您的 Azure Database for PostgreSQL 实例上**启用 Microsoft Entra 认证**并向您的工作负载身份授予访问权限
...
<Note>
上面列出的所有作业（除了 `e2eTest`）都使用 `backend` 服务账户。对于这些作业，您只需要配置 pod 标签（Azure 要求在 pod 上设置 `azure.workload.identity/use: "true"`）。`e2eTest` 作业使用自己的服务账户，需要单独的注解配置。
</Note>
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\self-host-external-postgres.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
