...
从一开始就设置好可观测性，可以帮助你比原本可能的速度**快得多**地进行迭代。它让你在快速迭代提示词，或更改使用的数据和模型时，能够清晰地洞察你的应用。在本节中，我们将介绍如何设置可观测性，以便你在原型设计阶段获得最大的可见性。
...
![追踪教程 openai](/langsmith/images/tracing-tutorial-openai.png)

### 追踪整个链

很好——我们已经追踪了 LLM 调用。但追踪更多内容通常非常有价值。LangSmith **专为**追踪整个 LLM 流水线而构建——让我们开始吧！我们可以通过修改代码来实现，使其现在看起来像这样：
...
![追踪教程链](/langsmith/images/tracing-tutorial-chain.png)
...
一旦反馈被记录，你就可以通过点击检查运行时的 `Metadata` 选项卡，看到它与每次运行的关联。它应该看起来像[这样](https://smith.langchain.com/public/8cafba6a-1a6d-4a73-8565-483186f31c29/r)
...
注意我们向 `rag` 函数添加了 `@traceable(metadata={"llm": "gpt-4.1-mini"})`。
...
现在我们已经记录了这两条元数据，我们应该能够在[追踪教程追踪 UI](https://smith.langchain.com/public/37adf7e5-97aa-42d0-9850-99c0199bddf6/r) 中看到它们都显示出来。
...
在生产环境中，你可能拥有**更多**的流量。你不想被困在逐个查看数据点上。幸运的是，LangSmith 有一套工具来帮助实现生产环境中的可观测性。
...
你还可以使用此选项卡执行一种 A/B 测试。在之前的教程中，我们开始跟踪几个不同的元数据属性——其中之一是 `llm`。我们可以按**任何**元数据属性对监控图表进行分组，并立即获得随时间变化的分组图表。这让我们可以尝试不同的 LLM（或提示词，或其他），并跟踪它们随时间变化的性能。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\observability-llm-tutorial.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
