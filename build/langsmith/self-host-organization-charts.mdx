...
* 按工作区统计的使用量：此图表按工作区统计追踪（根运行）数量
* 组织使用量：此图表统计组织内所有追踪（根运行）数量

图表每 5 分钟刷新一次，以包含任何新的工作区。请注意，这些图表不可编辑。

## 以编程方式获取追踪数量

您可以使用两种不同的方法以编程方式获取追踪数量：

### 方法一：使用 LangSmith REST API
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\self-host-organization-charts.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
