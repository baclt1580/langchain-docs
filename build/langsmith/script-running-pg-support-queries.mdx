...
此 Helm 仓库包含用于生成 LangSmith UI 目前不直接支持的输出的查询（例如，在单个查询中获取多个组织的追踪计数）。

该命令接受一个包含嵌入式名称和密码的 PostgreSQL 连接字符串（可以从密钥管理器调用中传入），并执行输入文件中的查询。在下面的示例中，我们使用 `support_queries/postgres` 目录中的 `pg_get_trace_counts_daily.sql` 输入文件。
...
也可以直接对 PostgreSQL 数据库运行 SQL 脚本以导出使用数据。这需要数据库访问凭据——不适用应用程序级别的权限。

要导出追踪使用情况（需要 Helm chart 版本 0.11.4 或更高）：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\script-running-pg-support-queries.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
