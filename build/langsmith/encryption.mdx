...
| 方法 | 加密内容 | 使用场景 |
|--------|------------------|----------|
| **基础加密** | 检查点二进制大对象，可选 JSON 字段 | 单一静态密钥，自动 AES 加密 |
...
1. 在 `langgraph.json` 中[配置](#配置)加密模块路径
2. [定义您的加密模块](#定义您的加密模块)，包含用于二进制大对象和 JSON 加密的处理程序
3. 通过 `X-Encryption-Context` 头[传递加密上下文](#传递加密上下文)（如租户 ID）
4. LangGraph 在存储前和检索后调用您的处理程序

对于需要密钥轮换和审计日志记录的生产环境部署，请参阅[使用 AWS Encryption SDK 的信封加密](#使用-aws-encryption-sdk-的信封加密)。
...
二进制大对象处理程序加密检查点数据——图执行序列化后的状态。以下是一个使用按租户密钥和 [Fernet](https://cryptography.io/en/latest/fernet/)（来自 `cryptography` 库的对称加密方案）的简化示例：
...
- 运行级执行限制（`max_concurrency`、`recursion_limit`），在运行的 `config` 中指定
...
**二进制大对象处理程序**（`@encryption.encrypt.blob` / `@encryption.decrypt.blob`）应用于检查点二进制大对象（图执行状态）。
...
通过 `X-Encryption-Context` 头传递加密上下文。上下文是您定义的任意数据——您控制其模式，可以包含加密逻辑所需的任何字段（例如 `tenant_id`、`key_version`）。上下文在您的处理程序中作为 `ctx.metadata` 可用，并以明文存储，供解密时使用。
...
对于 AWS 上的生产环境部署，请使用 [AWS Encryption SDK](https://docs.aws.amazon.com/encryption-sdk/latest/developer-guide/python.html) 配合 AWS KMS，或在您的云提供商内使用等效方案。此方法：
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\encryption.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
