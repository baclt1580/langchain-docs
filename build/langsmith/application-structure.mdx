...
1.  一个[配置文件](#configuration-file-concepts) (`langgraph.json`)，用于指定应用所需的依赖项、图和环境变量。
2.  实现应用逻辑的[图](#graphs)。
...
2.  [配置文件](#configuration-file-concepts) 中的 `dependencies` 键，用于指定运行应用所需的依赖项。
3.  任何额外的二进制文件或系统库可以使用 [LangGraph 配置文件](#configuration-file-concepts) 中的 `dockerfile_lines` 键来指定。

## 图

使用[配置文件](#configuration-file-concepts)中的 `graphs` 键来指定部署的应用中将提供哪些图。
...
<Tabs>
  <Tab title="Python">
    ...
    1.  **一个 LangGraph 图结构**：使用 [`StateGraph`](https://reference.langchain.com/python/langgraph/graph/state/StateGraph) 以及 [`add_node`](https://reference.langchain.com/python/langgraph/graph/state/StateGraph/add_node) 和 [`add_edge`](https://reference.langchain.com/python/langgraph/pregel/_draw/add_edge) 定义一个图。
    1.  **包含任意逻辑的节点函数**：您的节点函数可以调用任何框架或库。
    1.  **一个已编译的图**：[编译](https://reference.langchain.com/python/langgraph/graph/state/StateGraph/compile) 该图以创建可部署的应用。
  ...
  <Tab title="JavaScript">
    ...
    1.  **包含任意逻辑的节点函数**：您的节点函数可以调用任何框架或库。
    1.  **一个已编译的图**：[编译](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#compile) 该图以创建可部署的应用。
...
如果您正在[本地](/langsmith/local-dev-testing)处理已部署的 LangGraph 应用，可以在[配置文件](#configuration-file-concepts)的 `env` 键中配置环境变量。
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/i18n\zh-CN\langsmith\application-structure.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
