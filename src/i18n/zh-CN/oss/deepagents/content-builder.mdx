...
本指南演示如何使用 [Deep Agents](/oss/deepagents) 从零开始构建一个内容写作智能体。
...
-   [长期记忆](/oss/deepagents/long-term-memory) 用于 TODO
-   [技能](/oss/deepagents/skills) 用于 TODO
-   [子智能体](/oss/deepagents/subagents) 用于 TODO
-   [文件系统后端](/oss/deepagents/backends) 用于文件读写
...
在项目根目录创建 `AGENTS.md`。
当您稍后创建智能体并将此文件指定为 [记忆](/oss/deepagents/long-term-memory) 参数的一部分时，它会被加载到系统提示中，从而使品牌语调和研究要求适用于每次运行。
...
:::python
使用 @[create_deep_agent] 创建深度智能体时，传递记忆路径、技能目录、图像工具、来自 YAML 的子智能体，以及一个根目录为示例目录的 [FilesystemBackend](/oss/deepagents/backends)，以便像 `./AGENTS.md` 和 `./skills/` 这样的路径能够正确解析。

<ContentBuilderCreateAgentPy />
:::

:::js
使用 @[createDeepAgent] 创建深度智能体时，传递记忆路径、技能目录、图像工具、内联子智能体定义，以及一个根目录为示例目录的 [FilesystemBackend](/oss/deepagents/backends)，以便像 `./AGENTS.md` 和 `./skills/` 这样的路径能够正确解析。
...
-   阅读 [子智能体](/oss/deepagents/subagents)、[技能](/oss/deepagents/skills) 和 [自定义](/oss/deepagents/customization) 以进行更深入的配置