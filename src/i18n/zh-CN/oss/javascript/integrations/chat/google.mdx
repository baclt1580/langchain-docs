...
这将帮助您开始使用 `ChatGoogle` [聊天模型](/oss/langchain/models)。有关 `ChatGoogle` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-google/index/ChatGoogle)。

<Tip>
  `@langchain/google` 是所有新 Google Gemini 集成的推荐包。
  它取代了旧的 [`@langchain/google-genai`](/oss/integrations/chat/google_generative_ai) 和 [`@langchain/google-vertexai`](/oss/integrations/chat/google_vertex_ai) 包。
  有关迁移详情，请参阅 [遗留包](/oss/integrations/providers/google#legacy-packages)。
</Tip>
...
{/* TODO: 添加 enableEnhancedCivicAnswers 部分 */}
...
`ChatGoogle` 支持标准的 LangChain [工具调用](/oss/langchain/tools)，以及 Gemini 特定的“专业工具”（如代码执行和 Grounding）。
