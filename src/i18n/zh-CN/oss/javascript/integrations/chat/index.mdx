...
[聊天模型](/oss/langchain/models) 是以一系列[消息](/oss/langchain/messages)作为输入并返回消息作为输出的语言模型 <Tooltip tip="较旧的模型不遵循聊天模型接口，而是使用以字符串作为输入并返回字符串作为输出的接口。这些模型通常名称中不包含 'Chat' 前缀，或者包含 'LLM' 作为后缀。">（与纯文本相对）</Tooltip>。
...
某些模型提供商提供与 OpenAI（旧版）[聊天补全 API](https://platform.openai.com/docs/guides/completions) 兼容的端点。在这种情况下，您可以使用带有自定义 `base_url` 的 [`ChatOpenAI`](/oss/integrations/chat/openai) 来连接到这些端点。请注意，基于聊天补全 API 构建的功能可能不被 `ChatOpenAI` 完全支持；在这种情况下，请考虑使用特定于提供商的类（如果可用）。
