...
npm install @langchain/azure-cosmosdb @langchain/core
```
查看[使用示例](/oss/integrations/vectorstores/azure_documentdb)。
...
> 语义缓存功能通过 Azure Cosmos DB for NoSQL 集成得到支持，使用户能够基于用户输入与先前缓存结果之间的语义相似性来检索缓存响应。它利用了 [AzureCosmosDBNoSQLVectorStore](/oss/integrations/vectorstores/azure_cosmosdb_nosql)，该存储库存储缓存提示的向量嵌入。这些嵌入支持基于相似性的搜索，使系统能够检索相关的缓存结果。
...
```
查看 [Azure Blob 存储容器的使用示例](/oss/integrations/document_loaders/web_loaders/azure_blob_storage_container)。

```typescript
import { AzureBlobStorageContainerLoader } from "@langchain/community/document_loaders/web/azure_blob_storage_container";
```
查看 [Azure 文件存储的使用示例](/oss/integrations/document_loaders/web_loaders/azure_blob_storage_file)。
