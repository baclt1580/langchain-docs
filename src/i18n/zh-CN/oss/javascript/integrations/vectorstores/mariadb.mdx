...
本指南提供了快速入门 MariaDB [向量存储](/oss/integrations/vectorstores) 的概述。有关 `MariaDBStore` 所有功能和配置的详细文档，请参阅 [API 参考](https://reference.langchain.com/javascript/langchain-community/vectorstores/mariadb/MariaDBStore)。
...
本指南还将使用 [OpenAI 嵌入](/oss/integrations/embeddings/openai)，这需要你安装 `@langchain/openai` 集成包。你也可以使用 [其他支持的嵌入模型](/oss/integrations/embeddings)。
