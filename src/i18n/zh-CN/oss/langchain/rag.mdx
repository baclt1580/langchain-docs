...
由大语言模型（LLM）支持的最强大的应用之一是复杂的问答（Q&A）聊天机器人。这些应用程序可以回答关于特定源信息的问题。这些应用程序使用一种称为检索增强生成（Retrieval Augmented Generation，或 [RAG](/oss/langchain/retrieval/)）的技术。
...
一旦我们索引了数据，我们将使用 [代理](/oss/langchain/agents) 作为我们的编排框架来实现检索和生成步骤。
...
如果您的数据已经索引并可用于搜索（即您有一个执行搜索的函数），或者如果您熟悉 [文档加载器](/oss/integrations/document_loaders)、[嵌入](/oss/integrations/embeddings) 和 [向量存储](/oss/integrations/vectorstores)，请随时跳到下一节 [检索和生成](/oss/langchain/rag#2-retrieval-and-generation)。

</Note>

索引通常按以下方式工作：

1. **加载**：首先我们需要加载数据。这是通过 [文档加载器](/oss/integrations/document_loaders) 完成的。
2. **拆分**：[文本拆分器](/oss/integrations/splitters) 将大型 `Documents` 拆分为较小的块。这对于索引数据和将其传递给模型都有用，因为大块更难搜索且无法适应模型的有限上下文窗口。
3. **存储**：我们需要一个地方来存储和索引我们的拆分，以便以后可以搜索它们。这通常使用 [向量存储](/oss/integrations/vectorstores) 和 [嵌入](/oss/integrations/embeddings) 模型来完成。

![index_diagram](/images/rag_indexing.png)

### 加载文档

我们首先需要加载博客文章内容。我们可以为此使用 [DocumentLoaders](/oss/integrations/document_loaders)，它们是加载源数据并返回 @[Document] 对象列表的对象。
...
- [集成](/oss/integrations/document_loaders/)：160+ 种集成可供选择。
...
为了处理这个问题，我们将把 @[`Document`] 拆分为用于嵌入和向量存储的块。这将有助于我们在运行时仅检索博客文章中最相关的部分。

与 [语义搜索教程](/oss/langchain/knowledge-base) 一样，我们使用 `RecursiveCharacterTextSplitter`，它将使用常见的分隔符（如新行）递归地拆分文档，直到每个块大小合适。这是用于通用文本用例的推荐文本拆分器。
...
`TextSplitter`：将 @[`Document`] 对象列表拆分为较小块以供存储和检索的对象。
...
现在我们需要索引我们的 66 个文本块，以便在运行时搜索它们。遵循 [语义搜索教程](/oss/langchain/knowledge-base)，我们的方法是将每个文档拆分的内容 [嵌入](/oss/integrations/embeddings) 并将这些嵌入插入到 [向量存储](/oss/integrations/vectorstores) 中。给定输入查询，我们可以随后使用向量搜索检索相关文档。

我们可以使用在 [教程开始时](/oss/langchain/rag#components) 选择的向量存储和嵌入模型，在一个命令中嵌入并存储所有文档拆分。
...
- [集成](/oss/integrations/embeddings/)：30+ 种集成可供选择。
- @[接口][Embeddings]: 基础接口的 API 参考。

`VectorStore`：围绕向量数据库的包装器，用于存储和查询嵌入。

- [集成](/oss/integrations/vectorstores/)：40+ 种集成可供选择。
...
1. **检索**：给定用户输入，使用 [检索器](/oss/integrations/retrievers) 从存储中检索相关拆分。
2. **生成**：[模型](/oss/langchain/models) 使用包含问题和检索数据的提示生成答案。
...
RAG 应用程序的一种形式是一个带有检索信息工具的简单 [代理](/oss/langchain/agents)。我们可以通过实现一个包装我们的向量存储的 [工具](/oss/langchain/tools) 来组装一个最小的 RAG 代理：
...
在这里，我们使用 @[工具装饰器][@tool] 配置工具，将原始文档作为 [工件](/oss/langchain/messages#param-artifact) 附加到每个 [ToolMessage](/oss/langchain/messages#tool-message)。这将使我们在应用程序中访问文档元数据，而无需发送字符串化表示给模型。
...
```
<Tip>
    在这里，我们将 `responseFormat` 指定为 `content_and_artifact`，以配置工具将原始文档作为 [工件](/oss/langchain/messages#param-artifact) 附加到每个 [ToolMessage](/oss/langchain/messages#tool-message)。这将使我们在应用程序中访问文档元数据，而无需发送字符串化表示给模型。
</Tip>
:::
...
<Tip>
    您可以直接使用 [LangGraph](/oss/langgraph/overview) 框架添加更深层次的控制和自定义——例如，您可以添加步骤来评估文档相关性并重写搜索查询。查看 LangGraph 的 [Agentic RAG 教程](/oss/langgraph/agentic-rag) 以了解更高级的公式。
</Tip>


### RAG 链

在上述 [代理式 RAG](#rag-agents) 公式中，我们允许 LLM 自行决定生成 [工具调用](/oss/langchain/models#tool-calling) 以帮助回答用户查询。这是一个良好的通用解决方案，但也带来了一些权衡：
...
1. 添加一个键来存储检索到的文档
2. 通过 [中间件钩子](/oss/langchain/middleware/custom#node-style-hooks) 添加一个新节点，例如 `before_model` 来填充该键（以及注入上下文）。
...
既然我们已经通过 @[`create_agent`] 实现了简单的 RAG 应用程序，我们可以轻松添加新功能并深入探索：
...
- [流式传输](/oss/langchain/streaming) token 和其他信息以实现响应的用户体验
