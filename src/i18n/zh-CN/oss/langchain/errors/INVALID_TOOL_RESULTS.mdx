...
* **重复响应**：为同一个工具调用 ID 提供多个 @[`ToolMessage`] 对象会导致拒绝，未匹配的 ID 也会导致同样问题
* **孤立工具消息**：发送 @[`ToolMessage`] 而没有包含工具调用的前置 @[`AIMessage`] 违反了协议要求
...
* **匹配计数**：确保前置 @[`AIMessage`] 中的每个工具调用都有一个对应的 @[`ToolMessage`]
