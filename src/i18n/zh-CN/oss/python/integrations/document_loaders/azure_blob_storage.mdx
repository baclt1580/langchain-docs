...
目前，解析每个 Blob 时的默认行为是返回一个 @[`Document`] 对象，内容采用 UTF-8 编码，无论文件类型如何。对于需要特定解析的文件类型（例如 PDF、CSV 等），或者当您希望控制文档内容格式时，可以提供 `loader_factory` 参数，该参数接受现有的文档加载器（例如 PyPDFLoader、CSVLoader 等）或自定义加载器。

此功能通过将 Blob 内容下载到临时文件来实现。然后，`loader_factory` 会接收文件路径，使用指定的文档加载器加载/解析文件并返回 @[`Document`] 对象。
