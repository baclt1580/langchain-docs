...
语义缓存允许用户根据用户输入与先前缓存输入之间的语义相似性检索缓存的提示。底层它同时使用 Couchbase 作为缓存和向量存储。CouchbaseSemanticCache 需要定义一个 Search Index 才能工作。请查看 [使用示例](/oss/integrations/vectorstores/couchbase) 了解如何设置索引。
