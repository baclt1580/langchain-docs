...
查看 [`ChatNVIDIA` 集成页面](/oss/integrations/chat/nvidia_ai_endpoints) 以获取完整文档，包括工具调用、多模态输入和 Nemotron 特定示例。
...
查看 [`ChatNVIDIA` 集成页面](/oss/integrations/chat/nvidia_ai_endpoints#use-with-nvidia-dynamo) 以获取完整的 `ChatNVIDIADynamo` 参考，包括每次调用的覆盖和流式传输。
...
查看 [`NVIDIAEmbeddings` 集成页面](/oss/integrations/embeddings/nvidia_ai_endpoints) 以获取完整文档。
...
查看 [`NVIDIARAGRetriever` 集成页面](/oss/integrations/retrievers/nvidia) 以获取完整文档。
...
- [`NVIDIAEmbeddings` RAG 工作流模型](/oss/integrations/embeddings/nvidia_ai_endpoints)
