...
3. 📊 **MLflow** - [MLflow](https://mlflow.org/) 是一个开源平台，用于管理完整的 ML 生命周期，包括实验管理、评估、追踪、部署等。[MLflow 的 LangChain 集成](/oss/integrations/providers/mlflow_tracking) 简化了现代复合 ML 系统的开发和运营过程。
...
请参阅 [使用示例](/oss/integrations/chat/databricks) 以获取更多关于如何在 LangChain 应用程序中使用它的指导。
...
<Warning>
文本补全模型已被弃用，最新和最流行的模型是 [聊天补全模型](/oss/langchain/models)。请使用 `ChatDatabricks` 聊天模型来使用这些模型及高级功能（如工具调用）。
</Warning>
...
请参阅 [使用示例](/oss/integrations/llms/databricks) 以获取更多关于如何在 LangChain 应用程序中使用它的指导。
...
请参阅 [使用示例](/oss/integrations/embeddings/databricks) 以获取更多关于如何在 LangChain 应用程序中使用它的指导。
...
请参阅 [MLflow LangChain 集成](/oss/integrations/providers/mlflow_tracking) 以通过广泛的代码示例和指南了解使用 MLflow 与 LangChain 的全部功能。
