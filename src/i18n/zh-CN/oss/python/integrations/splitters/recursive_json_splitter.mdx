...
此 JSON 分割器 [拆分](/oss/integrations/splitters/) JSON 数据，同时允许控制块大小。它深度优先遍历 JSON 数据并构建较小的 JSON 块。它尝试保持嵌套的 JSON 对象完整，但如果需要，为了将块保持在 min_chunk_size 和 max_chunk_size 之间，也会拆分它们。
