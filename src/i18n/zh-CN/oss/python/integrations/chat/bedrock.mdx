...
当使用 Anthropic 模型进行[工具调用](/oss/langchain/tools)或[结构化输出](/oss/langchain/structured-output)时，工具调用的参数默认以部分 JSON 块的形式流式传输。
...
如果在输入文档上启用了引用功能，则可以生成引用。文档可以以 Bedrock 的[原生格式](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_DocumentBlock.html)或 LangChain 的[标准类型](/oss/langchain/messages#multimodal)指定：
...
有关可用参数和功能的详细文档，请参阅 [`ChatAnthropic` 集成页面](/oss/integrations/chat/anthropic)。
