...
| [工具调用](/oss/langchain/tools) | [结构化输出](/oss/langchain/structured-output) | 图像输入 | 音频输入 | 视频输入 | [令牌级流式传输](/oss/langchain/streaming/) | 原生异步 | [令牌用量](/oss/langchain/models#token-usage) | [日志概率](/oss/langchain/models#log-probabilities) |
| :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
...
您可以在单个模型调用中访问此功能，或者通过指定 LangChain [代理](/oss/langchain/agents) 的 [响应格式](/oss/langchain/structured-output)。示例如下。

<Accordion title="单个模型调用">

使用 [`with_structured_output`](/oss/langchain/models#structured-output) 方法生成结构化的模型响应。指定 `method="json_schema"` 以启用 OpenAI 的原生结构化输出功能；否则该方法默认使用函数调用。
...
指定 `response_format` 并使用 [`ProviderStrategy`](/oss/langchain/agents#providerstrategy) 在生成最终响应时启用 OpenAI 的结构化输出功能。
...
请注意，响应包含结构化的 [内容块](/oss/langchain/messages/#message-content)，其中包括响应的文本和 OpenAI [引用](https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses#output-and-citations) 来源。输出消息还将包含任何工具调用的信息：
...
有关更多详细信息，请参阅 [流式传输指南](/oss/langchain/streamming/)。
...
以下示例展示了如何使用 [自定义中间件](/oss/langchain/middleware/custom) 编排此操作。该示例实现了定义搜索逻辑的可调用对象。中间件随后包括：
1. `after_model` 钩子以检查 `tool_search_call` 块并调用我们的可调用对象
2. `wrap_tool_call` 钩子用于 [运行时工具注册](/oss/langchain/agents#runtime-tool-registration)
...
@[`ChatOpenAI`] 支持 `"computer-use-preview"` 模型，这是一个专为内置计算机使用工具设计的专用模型。要启用，像传递其他工具一样传递 [计算机使用工具](https://platform.openai.com/docs/guides/tools-computer-use)。
...
您可以手动管理状态或使用 [LangGraph](/oss/langgraph/quickstart)，与其他聊天模型一样：
...
<Tip>
**您可以使用 [LangGraph](https://langchain-ai.github.io/langgraph/) 在各种后端（包括内存和 Postgres）中为您管理对话线程。请参阅 [本教程](/oss/langgraph/quickstart) 开始使用。**
...
```
启用后，`AIMessage` 响应可能在内容中包含类型为 `"compaction"` 的块。这些应保留在对话历史记录中，并可以按 [通常方式](/oss/langchain/short-term-memory) 附加到消息序列。最近的 `compaction` 项之前的消息可以保留，也可以丢弃以提高延迟。
...
OpenAI 可以返回模型推理的摘要（尽管它不暴露原始推理令牌）。要配置 @[`ChatOpenAI`] 返回此摘要，请指定 `reasoning` 参数。如果设置了此参数，@[`ChatOpenAI`] 将自动路由到 Responses API。
...
OpenAI 有支持多模态输入的模型。您可以将这些模型传递图像、PDF 或音频。有关如何在 LangChain 中执行此操作的更多信息，请前往 [多模态输入](/oss/langchain/messages#multimodal) 文档。

您可以在 [OpenAI 文档](https://platform.openai.com/docs/models) 中查看支持不同模态的模型列表。

对于所有模态，LangChain 都支持其跨提供商标准以及 OpenAI 的原生内容块格式。

要将多模态数据传递给 @[`ChatOpenAI`]，请创建包含数据的 [内容块](/oss/langchain/messages/) 并将其合并到消息中，例如如下：
...
请参阅 [多模态消息操作指南](/oss/langchain/messages#multimodal) 中的示例。
...
阅读有关 [OpenAI 多模态消息的文件名](/oss/langchain/messages#multimodal) 的更多信息。

请参阅 [PDF 文档操作指南](/oss/langchain/messages#multimodal) 中的示例。
...
请参阅 [音频操作指南](/oss/langchain/messages#multimodal) 中的示例。
