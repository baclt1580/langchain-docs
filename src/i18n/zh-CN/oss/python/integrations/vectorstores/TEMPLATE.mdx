...
description: "使用 LangChain Python 与 (MODULE_NAME) 向量存储进行集成。"
---
---
...
本指南提供快速了解 (MODULE_NAME) [vector store](/oss/integrations/vectorstores#overview) 的概述。有关所有 (MODULE_NAME)VectorStore 功能、参数和配置的详细列表，请前往 [(MODULE_NAME)VectorStore API reference](<LINK>)。

## 设置

要访问 (MODULE_NAME) 向量存储，您需要创建一个 (MODULE_NAME) 账户，获取 API 密钥，并安装 `langchain-(MODULE_NAME)` 集成包。
...
要为您的模型调用启用自动 <Tooltip tip="记录模型执行的每一步以便调试和改进它">追踪</Tooltip>，请设置您的 [LangSmith](/langsmith/home) API 密钥：
...
一旦创建好您的向量存储并添加了相关文档，您很可能希望在运行链或代理期间查询它。
...
示例：
- <Icon icon="ruler" size={16}/> 距离函数
- <Icon icon="chart-bar" size={16}/> 相关性
- <Icon icon="filter" size={16}/> 过滤
- <Icon icon="server" size={16} /> 外部存储选项
- <Icon icon="tags" size={16} /> 索引
- <Icon icon="git-merge" size={16} /> 合并

---

## API 参考

有关所有 (MODULE_NAME)VectorStore 功能和配置的详细文档，请前往 [API reference](<LINK>)。