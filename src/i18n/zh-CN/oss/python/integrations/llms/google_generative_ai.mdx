...
<Warning>
**您当前所在的页面是关于将 Google 模型用作文本补全模型的文档。许多流行的 Google 模型是 [聊天完成模型](/oss/langchain/models)。**

您可能正在寻找 [此页面](/oss/integrations/chat/google_generative_ai/)。
</Warning>

关于使用 [Google Generative AI](https://developers.generativeai.google/) 模型与 LangChain 的指南。注意：它与 Google Cloud Vertex AI [集成](/oss/integrations/llms/google_vertex_ai) 是分开的。
