...
如上所述，我们将定义两个评估器：一个评估检索到的文档相对于输入查询的相关性，另一个评估生成的答案相对于检索到的文档的幻觉程度。我们将使用LangChain LLM包装器以及 @[`with_structured_output`][BaseChatModel.with_structured_output] 来定义幻觉评估器。
