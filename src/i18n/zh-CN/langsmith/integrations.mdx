...
[LangSmith](https://smith.langchain.com/) 提供了与日益增多的流行 [LLM 提供商](#llm-提供商) 和 [智能体框架](#智能体框架) 的集成，同时也支持 [Deep Agents](/oss/deepagents/overview)、[LangChain](/oss/langchain/overview) 和 [LangGraph](/oss/langgraph/overview)。关于设置和使用方法，请参考本页列出的指南。
...
**使用 Langchain？** LangChain 为 100 多个 LLM 提供商提供了统一接口，允许您通过设置环境变量来切换模型。[初始化一个模型](/oss/langchain/models#initialize-a-model)，LangSmith 将自动追踪您的应用程序。
</Callout>
