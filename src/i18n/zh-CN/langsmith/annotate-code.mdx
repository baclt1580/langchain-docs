...
推荐的方法是使用 @[`@traceable`] 装饰器（Python）或 [`traceable`](https://reference.langchain.com/javascript/langsmith/traceable) 包装器（TypeScript）。将其应用于任何函数，使其成为被追踪的运行，LangSmith 会自动处理嵌套调用间的上下文传播。
