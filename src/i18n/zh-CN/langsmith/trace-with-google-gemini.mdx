...
要追踪 Gemini API 调用，请使用 LangSmith 的 @[`wrap_gemini`][wrap_gemini]（Python）或 @[`wrapGemini`][wrapGemini]（JavaScript）包装函数。此包装器会拦截对 Gemini 客户端的调用，并自动将它们作为追踪记录到 LangSmith 中。包装器保留了原始客户端的全部功能，同时增加了可观测性：
...
您可以在调用 `wrap_gemini()` 时传递 @[`tracing_extra`][wrap_gemini] 来自定义追踪。此参数将应用于您使用该包装客户端进行的所有后续请求，从而允许您附加标签和元数据，以便在 [LangSmith UI](https://smith.langchain.com) 中对追踪进行筛选和组织。`tracing_extra` 参数接受：
...
您可以通过向 @[`wrapGemini`][wrapGemini] 传递配置选项来自定义追踪。这些选项将应用于您使用该包装客户端进行的所有后续请求，从而允许您附加标签和元数据，以便在 [LangSmith UI](https://smith.langchain.com) 中对追踪进行筛选和组织。配置接受：
