...
本指南展示如何遍历和处理从 @[`Client.evaluate()`][Client.evaluate] 返回的 @[`ExperimentResults`][ExperimentResults] 对象中的[实验](/langsmith/evaluation-concepts#experiment)结果。
...
@[`evaluate()`][Client.evaluate] 函数返回一个 @[`ExperimentResults`][ExperimentResults] 对象，你可以对其进行遍历。`blocking` 参数控制结果何时可用：
