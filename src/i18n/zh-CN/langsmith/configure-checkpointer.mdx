---
title: 配置检查点后端
sidebarTitle: 配置检查点后端
description: 配置 Agent Server 使用 PostgreSQL、MongoDB 或自定义实现来存储检查点。
---
...
<Note>
无论使用哪种检查点后端，LangSmith 始终需要 PostgreSQL 来存储线程、运行记录、助手、定时任务以及[内存存储](/oss/langgraph/persistence#memory-store)。检查点后端仅控制检查点数据的存储位置。
</Note>
...
要使用 PostgreSQL 或 MongoDB 之外的存储后端，请实现自定义的 @[BaseCheckpointSaver]。有关详细信息，请参阅[添加自定义检查点](/langsmith/custom-checkpointer)。
