...
在 LangSmith 中，身份验证由你的 @[`@auth.authenticate`][Auth.authenticate] 处理程序处理，授权由你的 @[`@auth.on`][Auth.on] 处理程序处理。
...
你在 LangGraph 中的 @[`@auth.authenticate`][Auth.authenticate] 处理程序处理步骤 4-6，而你的 @[`@auth.on`][Auth.on] 处理程序实现步骤 7。

## 身份验证

LangGraph 中的身份验证作为中间件在每个请求上运行。你的 @[`@auth.authenticate`][Auth.authenticate] 处理程序接收请求信息并应：
...
* 通过 @[`ctx.user`][AuthContext] 给你的授权处理程序
...
此对象包含有关当前用户的信息，包括你从 @[`@auth.authenticate`][Auth.authenticate] 处理程序返回的任何自定义字段。
...
有关如何向 MCP 服务器验证代理身份的信息，请参阅 [MCP 概念指南](/oss/langchain/mcp)。

## 授权

身份验证后，LangGraph 会调用你的 @[`@auth.on`][Auth] 处理程序来控制对特定资源（例如，线程、助手、定时任务）的访问。这些处理程序可以：
...
如果你只想实现简单的用户范围访问控制，可以为所有资源和操作使用单个 @[`@auth.on`][Auth] 处理程序。如果你想根据资源和操作进行不同的控制，可以使用[资源特定的处理程序](#资源特定的处理程序)。有关支持访问控制的完整资源列表，请参阅[支持的资源](#支持的资源)部分。
...
你可以通过将资源名称和操作名称与 @[`@auth.on`][Auth] 装饰器链接在一起来为特定资源和操作注册处理程序。
