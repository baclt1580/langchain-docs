...
本指南将向您展示如何创建、查看和检查**线程**。线程与[助手](/langsmith/assistants)协同工作，以实现您[已部署图](/langsmith/deployment)的[有状态](/oss/langgraph/persistence)执行。
...
要查看给定线程的当前状态，请使用 @[`get_state`][ThreadsClient.get_state] 方法。这将返回当前值、要执行的下一个节点和检查点信息：
...
更多信息，请参考 @[Python][ThreadsClient.get_state] 和 @[JS][ThreadsClient.get_state] SDK 文档，或 [REST API](/langsmith/agent-server-api/threads/get-thread-state) 参考。
...
要查看线程的历史记录，请使用 @[`get_history`][ThreadsClient.get_history] 方法。这将返回线程经历的所有状态的列表，允许您追踪完整的执行路径：
...
更多信息，请参考 @[Python][ThreadsClient.get_history] 和 @[JS][ThreadsClient.get_history] SDK 文档，或 [REST API](/langsmith/agent-server-api/threads/get-thread-history) 参考。
